...
void CGameFishingTable::OnUserHitFish(uint32 seat_id, uint32 bullet_id, uint32 bullet_bot, uint32 fish_no, uint64 time_stamp)
{
	...
	for (; iter != m_all_fish_info.end(); iter++)
	{
		...
		if (iter->fish_no == fish_no)
		{
			...
			bool flag = CFishCfgMgr::Instance().GetFishCfgInfo(m_pHostRoom, pPlayer, fish_type, ret, is_luck_ctrl, is_luck_win);
			...
			if (flag)
			{
				...
				LOG_DEBUG("BBB kill fish success - uid:%d seat_id:%d fish_type:%d ret:%d add_value:%d table_score:%lld set_score:%lld", 
					pPlayer->GetUID(), seat_id, fish_type, ret, add_value, m_lTableScore[seat_id], player_score + m_lTableScore[seat_id]);
