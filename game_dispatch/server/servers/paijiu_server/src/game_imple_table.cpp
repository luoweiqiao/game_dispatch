...
bool CGamePaijiuTable::ReAnalysisParam() {
	...
	LOG_ERROR("reader json parse success - roomid:%d,tableid:%d,m_robotBankerWinPro:%d,m_playerBankerLosePro:%d", m_pHostRoom->GetRoomID(), GetTableID(), m_robotBankerWinPro, m_playerBankerLosePro);
	return true;
}
    ...
    case GER_NORMAL:		//常规结束
        {
			...
			LOG_DEBUG("OnGameEnd2 roomid:%d,tableid:%d,lPlayerScoreResult:%lld,lRobotScoreResult:%lld,robotWinScore:%lld,playerFees:%lld,m_allJettonScore:%d-%d-%d-%d-%d-%d-%d--%d",
...
int64    CGamePaijiuTable::CalcPlayerInfo(uint32 uid,int64 winScore,bool isBanker)
{
...
bool CGamePaijiuTable::SetStockWinLose() {
	...
	uint8 cbTableCard[CARD_COUNT] = {0};
...
int64 CGamePaijiuTable::GetBankerAndPlayerWinScore() {
	...
	LOG_DEBUG("GetBankerAndPlayerWinScore - roomid:%d,tableid:%d,brankeruid:%d,playerAllWinScore:%lld,lBankerWinScore:%lld",
		m_pHostRoom->GetRoomID(), GetTableID(), brankeruid, playerAllWinScore, lBankerWinScore);
...
bool    CGamePaijiuTable::DispatchTableCard()
{
	...
	if (!bIsAreaControl && bBrankerIsControl && game_count>0 && (control_type == GAME_CONTROL_WIN || control_type == GAME_CONTROL_LOST))
	{
	...
	if (!bIsAreaControl && !bBrankerIsControl && control_uid != 0 && game_count>0 && control_type!= GAME_CONTROL_CANCEL)
	{
		for (uint8 i = 0; i < AREA_COUNT; ++i)
		{
	...
	if (!bIsAreaControl && bIsControlPlayerIsJetton && game_count>0 && control_type != GAME_CONTROL_CANCEL)
	{
		if (control_type == GAME_CONTROL_WIN)
		{
	...
	if (!bIsAreaControl && !bBrankerIsControl && !bIsControlPlayerIsJetton  && tmpJackpotScore.iUserJackpotControl == 1 && tmpJackpotScore.lCurPollScore>tmpJackpotScore.lMaxPollScore && bIsSysLostPro) // 吐币
	{
	...
	if (!bIsAreaControl && !bBrankerIsControl && !bIsControlPlayerIsJetton  && tmpJackpotScore.iUserJackpotControl == 1 && tmpJackpotScore.lCurPollScore<tmpJackpotScore.lMinPollScore && bIsSysWinPro) // 吃币
	{
		bIsPoolScoreControl = true;
		if (bBrankerIsRobot)
		{
	...
	if (!bIsAreaControl && !bBrankerIsControl && !bIsControlPlayerIsJetton && !bIsPoolScoreControl)
		bIsStockControl = SetStockWinLose();
	// add by har end

	if (!bIsAreaControl && !bBrankerIsControl && !bIsControlPlayerIsJetton &&!bIsPoolScoreControl && !bIsStockControl && needRobotChangeCard && bBrankerIsRobot)
	{
		SetRobotBrankerWin();
	}

	if (!bIsAreaControl && !bBrankerIsControl && !bIsControlPlayerIsJetton &&!bIsPoolScoreControl && !bIsStockControl && needPlayerChangeCard && !bBrankerIsRobot)
	{
		//设置真实庄家输
		SetPlayerBrankerLost();
	}
	SetIsAllRobotOrPlayerJetton(IsAllRobotOrPlayerJetton()); // add by har
	LOG_DEBUG("robot win - roomid:%d,tableid:%d,bIsAreaControl:%d,bBrankerIsRobot:%d,m_robotBankerWinPro:%d,needRobotChangeCard:%d,bIsControlPlayerIsJetton:%d,bIsStockControl:%d,GetIsAllRobotOrPlayerJetton:%d",
		GetRoomID(), GetTableID(), bIsAreaControl, bBrankerIsRobot, m_robotBankerWinPro, needRobotChangeCard, bIsControlPlayerIsJetton, bIsStockControl, GetIsAllRobotOrPlayerJetton());
...
uint32  CGamePaijiuTable::GetBankerUID()
{
    if(m_pCurBanker)
        return m_pCurBanker->GetUID();
    return 0;
}
//void    CGamePaijiuTable::RemoveApplyBanker(uint32 uid)
//{
...
//	}    
//}
bool    CGamePaijiuTable::LockApplyScore(CGamePlayer* pPlayer,int64 score)
{
...
bool    CGamePaijiuTable::UnLockApplyScore(CGamePlayer* pPlayer)
{
