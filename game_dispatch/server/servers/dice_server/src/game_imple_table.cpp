...
bool	CGameDiceTable::CalcuJackpotScore()
{
	...
	for (WORD wUserIndex = 0; wUserIndex < GAME_PLAYER; ++wUserIndex)
	{
		...
		int64 lValueFee = CalcPlayerInfo(pPlayer->GetUID(), lPlayerWinScore, lExWinScore);
...
bool CGameDiceTable::CalcuPlayerScore(int64 &playerAllWinScore)
{
	for (WORD wUserIndex = 0; wUserIndex < GAME_PLAYER; ++wUserIndex)
	{
		...
		// add by har
		//if (pPlayer->IsRobot())
		//	robotWinScore += lPlayerWinScore;
		//else
		//	playerFees -= lValueFee; // add by har end

		if (!pPlayer->IsRobot())
			playerAllWinScore += lWinScore; // add by har end

		//LOG_DEBUG("uid:%d,IsRobot:%d,lPoolAddScore:%lld,lPlayerWinScore:%lld,lPlayerJackpotScore:%lld,lExWinScore:%lld,lValueFee:%lld,lWinScore:%lld,robotWinScore:%d,playerFees:%lld",
		//	uid, pPlayer->IsRobot(), lPoolAddScore, lPlayerWinScore, lPlayerJackpotScore, lExWinScore, lValueFee, lWinScore, robotWinScore, playerFees);
...
{
    LOG_DEBUG("game end  roomid:%d,table:%d,m_wBankerTime:%d,reason:%d,chairID:%d", GetRoomID(), GetTableID(),m_wBankerTime, reason, chairID);
    switch(reason)
    {
    case GER_NORMAL:		//常规结束
        {
			...
			// add by har
			int64 robotWinScore = 0; // 机器人总赢分。玩家不能上庄，庄家的赢分就是机器人的赢分
   ...
   //         }
			...
			LOG_DEBUG("OnGameEnd2 - roomid:%d,tableid:%d,robotWinScore:%d,playerFees:%d,lBankerWinScore:%d", GetRoomID(), GetTableID(), robotWinScore, playerFees, lBankerWinScore);
...
int64 CGameDiceTable::CalculateScore()
{
    ...
    if(m_vecRecord.size() > MAX_SCORE_HISTORY)
	{
	...
	for (; it_looker != m_mpLookers.end(); it_looker++)
	{
		...
		LOG_DEBUG("score result looker - uid:%d,m_mpUserWinScore:%lld", dwUserID, m_mpUserWinScore[dwUserID]);

	}
    return lBankerWinScore;
}
int64    CGameDiceTable::CalcPlayerInfo(uint32 uid,int64 winScore, int64 lPlayerJackpotScore,bool isBanker)
{
	if (uid == 0 || winScore == 0)
	{
    ...
    int64 fee = CalcPlayerGameInfo(uid,winScore, lPlayerJackpotScore,isBanker?false:true);
	return fee;
...
bool    CGameDiceTable::DispatchTableCard()
{
	...
	if (m_GameLogic.m_bIsControlDice == false)
	{
		...
		//}
		// add by har
		if (!bIsFalgControlPlayer && !bIsPlayerNoWinJackpot && !bIsRobotSubJackpot)
			bIsStockControl = SetStockWinLose(); // add by har end

		if (!bIsFalgControlPlayer &&bNeedChangeCard && !bIsPlayerNoWinJackpot && !bIsRobotSubJackpot && !bIsStockControl)
		{
			bIsBrankerWin = SetRobotBrankerWin();
		}
	}
	SetIsAllRobotOrPlayerJetton(IsAllRobotOrPlayerJetton()); // add by har
	LOG_DEBUG("m_bIsControlDice:%d,bIsRobotSubJackpot:%d,bIsFalgControlPlayer:%d,bIsPlayerNoWinJackpot:%d,bRobotSubJackpotPro:%d,lMaxPollScore:%lld,lCurPollScore:%lld, bIsRobotSubJackpot:%d,bNeedChangeCard:%d,bIsBrankerWin:%d,bIsStockControl:%d,robotBankerWinPro:%d,GetIsAllRobotOrPlayerJetton:%d,m_cbTableDice: %d,%d,%d.",
		m_GameLogic.m_bIsControlDice, bIsRobotSubJackpot, bIsFalgControlPlayer, bIsPlayerNoWinJackpot, bRobotSubJackpotPro,
		tempDiceJackpotInfo.lMaxPollScore, tempDiceJackpotInfo.lCurPollScore, bIsRobotSubJackpot, bNeedChangeCard,
		bIsBrankerWin, bIsStockControl, robotBankerWinPro, GetIsAllRobotOrPlayerJetton(), m_cbTableDice[0], m_cbTableDice[1], m_cbTableDice[2]);
    return true;
}
