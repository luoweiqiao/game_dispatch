...
bool CGameTable::OnTableTick()
{
	...
	if (m_tagControlPalyer.uid != 0)
	{
		...
		//m_pHostRoom->GetRoomID(), GetTableID(), uSysTime, uOldTime, m_tagControlPalyer.uid, m_tagControlPalyer.count, m_tagControlPalyer.type);
...
bool CGameTable::UpdateControlMultiPlayer(uint32 uid, uint32 gamecount, uint64 gametime, int64 totalscore, uint32 operatetype)
{
	bool bIsRetFlag = true;
	if (uid == 0 || operatetype>GAME_CONTROL_MULTI_ALL_CANCEL)
	{
...
bool    CGameTable::EnterTable(CGamePlayer* pPlayer)
{
	...
	LOG_DEBUG("start enter table - roomid:%d,tableid:%d,uid:%d,IsRobot:%d", m_pHostRoom->GetRoomID(), GetTableID(), pPlayer->GetUID(), pPlayer->IsRobot());

    if(NeedSitDown()){//需要手动坐下站起
        pPlayer->SetTable(this);
		LOG_DEBUG("enter table - roomid:%d,tableid:%d,uid:%d,IsRobot:%d", m_pHostRoom->GetRoomID(), GetTableID(), pPlayer->GetUID(), pPlayer->IsRobot());
    ...
    {   //需要手动坐下站起
        for(uint8 i=0;i<m_vecPlayers.size();++i)
        {
            if(m_vecPlayers[i].pPlayer == pPlayer)
            {
				...
				LOG_DEBUG("chair level table - roomid:%d,tableid:%d,uid:%d,IsRobot:%d", m_pHostRoom->GetRoomID(), GetTableID(), pPlayer->GetUID(), pPlayer->IsRobot());
				//NotifyPlayerLeave(pPlayer);
                return true;
            }
        }
		LOG_DEBUG("look level table - roomid:%d,tableid:%d,uid:%d,IsRobot:%d", m_pHostRoom->GetRoomID(), GetTableID(), pPlayer->GetUID(), pPlayer->IsRobot());
...
bool    CGameTable::AddLooker(CGamePlayer* pPlayer)
{
	...
	LOG_DEBUG("add looker roomid:%d,tableid:%d,uid:%d,m_mpLookers.size:%d", GetRoomID(), GetTableID(),
		pPlayer->GetUID(), m_mpLookers.size());
    return true;
}
bool    CGameTable::RemoveLooker(CGamePlayer* pPlayer)
{
	...
	LOG_DEBUG("remove looker roomid:%d,tableid:%d,uid:%d,m_mpLookers.size:%d", GetRoomID(), GetTableID(),
		pPlayer->GetUID(), m_mpLookers.size());
    return true;   
}
bool    CGameTable::AddEveryColorLooker(CGamePlayer* pPlayer)
{
	...
	LOG_DEBUG("add every color looker roomid:%d,tableid:%d,uid:%d,m_mpLookers.size:%d", GetRoomID(), GetTableID(),
		pPlayer->GetUID(), m_mpLookers.size());
	return true;
}
bool    CGameTable::RemoveEveryColorLooker(CGamePlayer* pPlayer)
{
	if (pPlayer == NULL) return false;
	m_mpLookers.erase(pPlayer->GetUID());
	LOG_DEBUG("remove every color looker roomid:%d,tableid:%d,uid:%d,m_mpLookers.size:%d", GetRoomID(), GetTableID(),
		pPlayer->GetUID(), m_mpLookers.size());
	return true;
}
...
bool    CGameTable::CanEnterTable(CGamePlayer* pPlayer)
{
	...
	if (pPlayer->GetTable() != NULL) {
		LOG_DEBUG("player table is not null - uid:%d,ptable:%p,roomid:%d,tableid:%d,", pPlayer->GetUID(), pPlayer->GetTable(), GetRoomID(), GetTableID());
		return false;
	}
	if (IsFullTable() || GetGameState() != TABLE_STATE_FREE) {
		LOG_DEBUG("game table limit - uid:%d,is_full:%d,game_state:%d,roomid:%d,tableid:%d", pPlayer->GetUID(), IsFullTable(), GetGameState(), GetRoomID(), GetTableID());
		return false;
	}
    // 限额进入
    if(GetPlayerCurScore(pPlayer) < GetEnterMin()){
        LOG_DEBUG("min score limit - uid:%d,cur_score:%lld,min_score:%lld,roomid:%d,tableid:%d", pPlayer->GetUID(),GetPlayerCurScore(pPlayer),GetEnterMin(), GetRoomID(), GetTableID());
        return false;
    }
    if(pPlayer->IsRobot() && m_robotEnterCooling.isTimeOut() == false){// 冷却期机器人不能进入
        LOG_DEBUG("robot is cooling,do not enter table - uid:%d,roomid:%d,tableid:%d", pPlayer->GetUID(), GetRoomID(), GetTableID());
        return false;
    }

	LOG_DEBUG("player_can_enter_table - uid:%d,roomid:%d,tableid:%d,isrobot:%d", 
		pPlayer->GetUID(),GetRoomID(),GetTableID(), pPlayer->IsRobot());
...
bool    CGameTable::PlayerSitDownStandUp(CGamePlayer* pPlayer,bool sitDown,uint16 chairID)
{
    if(sitDown)//坐下
    {
        ...
        if(!CanStandUp(pPlayer)){
            LOG_DEBUG("can not standup in gameing  roomid:%d,tableid:%d,uid:%d", GetRoomID(), GetTableID(), pPlayer->GetUID());
            return false;
        }                
        for(uint8 i=0;i < m_vecPlayers.size();++i)
        {
...
void    CGameTable::SendTableInfoToClient(CGamePlayer* pPlayer)
{
    ...
    LOG_DEBUG("Send Table Info To Client - roomid:%d,tableid:%d,uid:%d",GetRoomID(),GetTableID(),pPlayer->GetUID());
}
void    CGameTable::SendReadyStateToClient()
{
    LOG_DEBUG("send ready state to client");
...
void    CGameTable::SendSeatInfoToClient(CGamePlayer* pGamePlayer)
{
	...
	LOG_DEBUG("Send SeatInfo to Client  roomid:%d,tableid:%d,m_vecPlayers.size:%d,m_mpLookers.size:%d",
		GetRoomID(), GetTableID(), m_vecPlayers.size(), m_mpLookers.size());
    if(pGamePlayer != NULL){
...
bool CGameTable::TableJettonLimmit(CGamePlayer * pPlayer, int64 lJettonScore, int64 lAllyJetton)
{
	...
	if (GetJettonMin() + lJettonScore + lAllyJetton > curScore)
	{
		return false;
	}
	return true;
...
void    CGameTable::FlushUserBlingLog(CGamePlayer* pPlayer,int64 winScore, int64 lJackpotScore,uint8 land)
{
    for(uint32 i=0;i<m_blingLog.users.size();++i)
	{
        if(pPlayer != NULL && m_blingLog.users[i].uid == pPlayer->GetUID())
		{
...
{
    LOG_DEBUG("Deduc End Fee  roomid:%d,tableid:%d,uid:%d,winScore:%d,feeType:%d", GetRoomID(), GetTableID(), uid, winScore, m_conf.feeType);
    int64 fee = 0;
...
// 上报游戏战报
void    CGameTable::ReportGameResult(uint32 uid,int64 winScore, int64 lExWinScore)
{
...
// 结算玩家信息
int64    CGameTable::CalcPlayerGameInfo(uint32 uid,int64& winScore, int64 lJackpotScore,bool bDeductFee)
{
    ...
    if(pPlayer != NULL)
    {
		...
		int64 lPlayerScore = winScore + lJackpotScore;
        ChangeScoreValueByUID(uid, lPlayerScore,emACCTRAN_OPER_TYPE_GAME,m_pHostRoom->GetGameType());
        pPlayer->AsyncChangeGameValue(m_pHostRoom->GetGameType(),isCoin, lPlayerScore, lJackpotScore);
        FlushUserBlingLog(pPlayer,winScore, lJackpotScore);

		OnSetPlayerWinScore(uid, lPlayerScore);
		UpdateMasterUserShowInfo(pPlayer, lPlayerScore);
		if (lJackpotScore > 0)
		{
		...
		if (GetPlayerCurScore(pPlayer) >= m_pHostRoom->GetEnterMin())
		{
			pPlayer->SetBankruptRecord(true);
		}
    }
	ReportGameResult(uid, winScore, lJackpotScore);
    return fee;
}
// 是否需要回收
...
{
	LOG_DEBUG("in_out_table - room:%d,tableid:%d,uid:%d,chairID:%d,isJoin:%d", m_pHostRoom->GetRoomID(), GetTableID(), pPlayer->GetUID(), chairID, isJoin);
...
bool CGameTable::UpdateControlPlayer(uint32 uid, uint32 gamecount, uint32 operatetype){
	if (uid == 0 || gamecount == 0 || operatetype>GAME_CONTROL_LOST)
	{
		LOG_ERROR("add control palyer failed - roomid:%d,tableid:%d,uid:%d,gamecount:%d,operatetype:%d", m_pHostRoom->GetRoomID(),GetTableID(), uid, gamecount, operatetype);
		return false;
	}
	...
	LOG_ERROR("add control palyer success - roomid:%d,tableid:%d,uid:%d,gamecount:%d,operatetype:%d", m_pHostRoom->GetRoomID(), GetTableID(), uid, gamecount, operatetype);
	return true;
}
...
bool CGameTable::SynControlPlayer(uint32 uid, int32 gamecount, uint32 operatetype) {
	...
	LOG_ERROR("syn control palyer success - roomid:%d,tableid:%d,uid:%d,gamecount:%d,operatetype:%d, m_tagControlPalyer.uid:%d,count:%d,type:%d",
		m_pHostRoom->GetRoomID(), GetTableID(), uid, gamecount, operatetype, m_tagControlPalyer.uid, m_tagControlPalyer.count, m_tagControlPalyer.type);
...
void CGameTable::KickLittleScoreUser()
{
	if (GetIsGameEndKickUser(GetGameType()))
	{
		...
		for (uint32 i = 0; i < m_vecPlayers.size(); i++)
		{
			...
			if (m_vecPlayers[i].pPlayer != NULL && lCurScore < 5000)
			{
...
void CGameTable::SaveUserBankruptScore()
{
	for (uint32 i = 0; i<m_blingLog.users.size(); ++i)
	{
		...
		if (m_blingLog.users[i].playerType == PLAYER_TYPE_ONLINE && m_pHostRoom != NULL && bIsJetton)
		{
			...
			if (m_blingLog.users[i].newValue < m_pHostRoom->GetEnterMin() && pGamePlayer != NULL && pGamePlayer->GetBankruptRecord())
			{
...
bool    CGameTable::IsBaiRenCount(uint16 gameType)
{
	switch (gameType)
	{
	case net::GAME_CATE_WAR:
...
bool    CGameTable::CalculateDeity()
{
	...
	if (IsBaiRenCount(GetGameType()) == false)
	{
		return false;
	}
...
void CGameTable::SendFrontJettonInfo(CGamePlayer * pPlayer)
{
	if (ContinuousPressureBaiRenGame())
	{
		if (m_spFrontPlayerJettonInfo != nullptr && pPlayer != NULL && pPlayer->IsRobot() == false)
		{
			...
			LOG_DEBUG("roomid:%d,tableid:%d,uid:%d,info_size:%d", GetRoomID(), GetTableID(), pPlayer->GetUID(), msg.info_size());
...
{
	if (IsBaiRenCount(GetGameType()) && m_mpBairenPalyerBet.size() > 0)
	{
...
void CGameTable::CheckRetireTableUser()
{
    if(GetTableType() == emTABLE_TYPE_PLAYER)
    {
        return;
    ...
    if(GetGameState() != TABLE_STATE_FREE)
    {
        LOG_ERROR("table:%d user is palying", m_tableID);
        return ;
    }
    ...
    if (CApplication::Instance().GetStatus() == emSERVER_STATE_RETIRE)
    {
        ...
        for (uint32 i = 0; i < m_vecPlayers.size(); ++i)
        {
            ...
            RetireUser(pGamePlayer);
        }
        
        //旁观玩家
        map<uint32,CGamePlayer*> mp_tmplook = m_mpLookers;
        map<uint32,CGamePlayer*>::iterator lk_iter = mp_tmplook.begin();
        for(; lk_iter != mp_tmplook.end(); ++lk_iter)
        {
            CGamePlayer * pGamePlayer = lk_iter->second;
            //RemoveLooker(pGamePlayer); 
            RetireUser(pGamePlayer);
        }
    }
}
...
int64	CGameTable::GetPlayerTotalWinScore(CGamePlayer *pPlayer)
{
	...
	if (pPlayer != NULL)
	{
		lPlayerTotalWinScore = pPlayer->GetPlayerTotalWinScore(CDataCfgMgr::Instance().GetCurSvrCfg().gameType);
	}
	return lPlayerTotalWinScore;
}

void    CGameTable::RetireUser(CGamePlayer *pPlayer)
{
    if (pPlayer == NULL)
    {
    ...
    if(pPlayer->IsRobot())
    {   
        LOG_ERROR("retire table:%d robot:%d", m_tableID, pPlayer->GetUID());
        CGameRobot * robot = static_cast<CGameRobot*>(pPlayer);
        CRobotMgr::Instance().RemoveRobot(robot);
    }
    else
    {
        LOG_ERROR("retire table:%d player:%d", m_tableID, pPlayer->GetUID());
        pPlayer->OnLoginOut();
...
bool CGameTable::GetIsMasterUser(CGamePlayer* pPlayer)
{
	...
	//LOG_DEBUG("master - roomid:%d,tableid:%d,suid:%d,tuid:%d,bIsMaster:%d",
	//	GetRoomID(), GetTableID(), uid, tagCtrlCfg.tuid, bIsMaster);
	*/
