...
int     CGameRobniuTable::OnGameMessage(CGamePlayer* pPlayer,uint16 cmdID, const uint8* pkt_buf, uint16 buf_len)
{
	...
	if (m_cbPlayStatus[chairID] == FALSE)
	{
		...
		net::msg_robniu_game_status msg;  // 编译报错 msg_robniu_game_status’不是‘net’的成员。解决办法：msg_define.proto 中加上 import "robniu_logic_msg.proto";
...
void CGameRobniuTable::SetCardDataControl()
{
	...
	bool bIsRobotWinScore = false;
	if (!m_bIsProgressControlPalyer && !m_bIsLuckyCtrl && !m_bIsNoviceWelfareCtrl && !bIsAWControl)
	{
	...
	if (!m_bIsProgressControlPalyer && !m_bIsLuckyCtrl && !m_bIsNoviceWelfareCtrl && !bIsAWControl && !bIsRobotWinScore)
