...
void	CPlayer::OnLoginOut(uint32 leaveparam)
{
	...
	if(IsPlaying() && !IsRobot())
	{
		...
		CRedisMgr::Instance().DelPlayerLobbySvrID(GetUID());
	}
...
void	CPlayer::OnLogin()
{
	...
	if (IsRobot() == false)
	{
		CRedisMgr::Instance().SetPlayerLobbySvrID(GetUID(), CApplication::Instance().GetServerID());
	}
	LOG_DEBUG("GetPlayerLobbySvrID - OnLogin - uid:%d,lobbyid:%d", GetUID(), CRedisMgr::Instance().GetPlayerLobbySvrID(GetUID()));
...
{	
    ...
    m_loginTime = getSysTime();
	CheckOtherLobbyServer();
	// 通知其他大厅服，保存该玩家的昵称等信息
	if (!IsRobot()) {
		net::msg_sync_other_lobby_player_data dispatchMsg;
		dispatchMsg.set_uid(GetUID());
		dispatchMsg.set_name(GetNickName());
		DispatchMgr::Instance().Broadcast2OtherLobby(&dispatchMsg, net::LD2L_MSG_SYNC_OTHER_LOBBY_PLAYER_DATA, 0, CApplication::Instance().GetServerID());
	}
	LOG_DEBUG("player login game server OnGetData player data - uid:%d,city:%s", GetUID(), GetCity().c_str());
}

void CPlayer::CheckOtherLobbyServer()
{
	if (IsRobot())
	{
		return;
	...
	msg.set_svrid(svrID);
	msg.set_uid(GetUID());
	DispatchMgr::Instance().SendMsg2DispatchSvr(&msg, net::LS2D_MSG_CHECK_OTHER_LOBBY_SERVER, svrID);
}
...
void	CPlayer::ReLogin()
{
	...
	LOG_DEBUG("GetPlayerLobbySvrID - ReLogin - uid:%d,svrid:%d,lobbyid:%d", GetUID(), GetCurSvrID(), CRedisMgr::Instance().GetPlayerLobbySvrID(GetUID()));
...
bool 	CPlayer::NeedRecover()
{
	...
	{
		LOG_ERROR("load player data time - uid:%d",GetUID());
		return true;
	}
	if (m_otherRecover)
	{
		LOG_ERROR("other server - uid:%d", GetUID());
		return true;
	}
	...
	if (CApplication::Instance().GetStatus() == emSERVER_STATE_REPAIR)
	{
		LOG_ERROR("repair server - uid:%d", GetUID());
		return true;
	}
...
bool	CPlayer::SendAccData2Client()
{
	...
	const net::base_info& playerInfo = msg.base_data();
	LOG_DEBUG("发送账号数据 - uid:%d,coin:%d", GetUID(), playerInfo.coin());
...
{
	LOG_DEBUG("更新数值到前端 - uid:%d,score:%lld,coin:%lld",GetUID(),GetAccountValue(emACC_VALUE_SCORE),GetAccountValue(emACC_VALUE_COIN));
...
{
	LOG_DEBUG("enter EnterGameSvr function. svrID:%d", svrID);

	if(GetCurSvrID() != 0 && GetCurSvrID() != svrID){
		//LOG_DEBUG("进入服务器失败:%d-->%d",GetCurSvrID(),svrID);
		LOG_DEBUG("enter_game_server_failed serverid error - uid:%d,curCerverID:%d,enterServerID:%d", GetUID(),GetCurSvrID(), svrID);
		return false;	
	}
	...
	if(pServer == NULL || pServer->status == emSERVER_STATE_REPAIR)
	{
		...
		LOG_DEBUG("enter_game_server_failed server not exist - uid:%d,curCerverID:%d,enterServerID:%d,pServer:%p,status:%d", GetUID(), GetCurSvrID(), svrID, pServer, status);
		return false;
	}

	if(pServer->status == emSERVER_STATE_RETIRE && GetCurSvrID() != svrID)	
	{
		LOG_DEBUG("gamesvr:%d is retiring...", svrID);
		return false;
	}
	...
	LOG_DEBUG("enter_game_server - svrid:%d,uid:%d,score:%lld,coin:%lld,win:%d,lose:%d,maxwinc:%lld,daywinc:%lld ctrl_flag:%d", 
		svrID,GetUID(),GetAccountValue(emACC_VALUE_SCORE),GetAccountValue(emACC_VALUE_COIN), msg.game_data().winc(), msg.game_data().losec(), msg.game_data().maxwinc(), msg.game_data().daywinc(), ctrl_flag);
...
bool    CPlayer::SyncChangeAccountValue(uint16 operType,uint16 subType,int64 diamond,int64 coin,int64 ingot,int64 score,int32 cvalue,int64 safecoin,const string& chessid)
{
	...
	if (ret == false)
	{
		LOG_DEBUG("end failed - uid:%d,operType:%d,coin:%lld,cur_coin:%lld", GetUID(), operType, coin, GetAccountValue(emACC_VALUE_COIN));
	...
	LOG_DEBUG("end success - uid:%d,operType:%d,coin:%lld,cur_coin:%lld", GetUID(), operType, coin, GetAccountValue(emACC_VALUE_COIN));
	return ret;
}
// 保存登陆奖励状态
...
bool 	CPlayer::GetBankruptHelp()
{
	...
	if(GetAccountValue(coinType) + GetAccountValue(emACC_VALUE_SAFECOIN) > CDataCfgMgr::Instance().GetBankruptBase()
	   ||  GetBankrupt() >= CDataCfgMgr::Instance().GetBankruptCount())
	{
