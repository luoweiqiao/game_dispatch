...
public:
    ...
    // 根据鱼的种类获取相应的配置信息
    bool    GetFishCfgInfo(CGameRoom *pHostRoom, CGamePlayer* pPlayer, uint32 fish_id, uint32 &ret, bool &is_luck_ctrl, bool &is_luck_win);
	...
	//inCInfo为输入的坐标值列表，outCInfo为输出的坐标值列表，allTime为总的执行时间 目前只需要支持输入三个坐标值
	//bool	GetBezierCurve(vector<CoordinateInfo> &inCInfo, vector<CoordinateInfo> &outCInfo, uint32 allTime, uint32 flushTime);
	//bool	fjfunction(vector<CoordinateInfo> pList, int32 tc, int32 index, vector<CoordinateInfo> &outCInfo);
