...
bool CGameNetMgr::Init()
{
    ...
    LobbyServer * lobbyServer = new LobbyServer("0.0.0.0", refCfg.svrport);
    int ret = CCReactor::Instance().RegistServer(lobbyServer);
    if (ret != 0)
    {
        LOG_ERROR("Regist lobby Server Failed.");
        return false;
    }
    ...
    LobbyHandle::m_pConnMng = new CObjSeg(sizeof(LobbyHandle), LOBBY_MAX_CONN_NUM, LobbyHandle::CreateObject);
    LobbyHandle::m_stConnHash.Initialize(LobbyHandle::m_pConnMng, 0);
...
int LobbyHandle::s_flow = 0;

IMPLEMENT_DYN(LobbyHandle)

CHashTab<LOBBY_MAX_CONN_NUM> LobbyHandle::m_stConnHash;
CObjSeg*    LobbyHandle::m_pConnMng;


int LobbyHandle::Init()
{
...
int LobbyHandle::OnPacketComplete(char * data, int len)
{
    CLobbyMgr::Instance().OnHandleClientMsg(this, (uint8_t *)data, len);
    //LOG_DEBUG("lobby handle recv pack len:%d", len);
...
void LobbyHandle::OnTimer()
{
    LOG_ERROR("lobby handle heart timeout this:%p uid:%d", this, GetUID());
    //! 心跳超时
    DestroyObj();
}

int LobbyHandle::DestroyObj()
{
    CCReactor::Instance().StopTimer(this);
    CLobbyMgr::Instance().RemoveServer(this);
    DetachPoller();
    DeleteNode(m_flow);
...
LobbyHandle* LobbyHandle::GetDRNodeByKey(unsigned int Key, int isCreate /*= True*/)
{
    LobbyHandle *pConn = (LobbyHandle *)LobbyHandle::m_stConnHash.GetObjectByKey((const void *)&Key, sizeof(Key));
    if (pConn == NULL)
    {
        if (isCreate)
        {
...
LobbyHandle* LobbyHandle::CreateDRNode(unsigned int Key)
{    
    if (LobbyHandle::m_pConnMng->GetFreeCount() == 0)
    {
        LOG_ERROR("create fail,not free node--used_cnt[%d],free_cnt[%d]",
            LobbyHandle::m_pConnMng->GetUsedCount(), LobbyHandle::m_pConnMng->GetFreeCount());
        return NULL;
    }

    LOG_DEBUG("create node used_cnt[%d],free_cnt[%d]", LobbyHandle::m_pConnMng->GetUsedCount(), LobbyHandle::m_pConnMng->GetFreeCount());

    LobbyHandle* pConn = NULL;
    int iErrNo = 0;
    pConn = (LobbyHandle *)LobbyHandle::m_stConnHash.CreateObjectByKey((const void *)&Key, sizeof(Key), iErrNo);
...
int LobbyHandle::DeleteNode(unsigned int Key)
{
    LobbyHandle* pConnNode = NULL;
    int iDelIdx = -1;
    ...
    if (pConnNode)
    {
        ...
        if (iDelIdx >= 0)
        {
            iDelIdx = LobbyHandle::m_stConnHash.DeleteObjectByKey((const void *)&abyTempKey[0], iTempKeyLength);

            if (iDelIdx < 0)
            {
                return -1;
            }
            LobbyHandle::m_pConnMng->DestroyObject(iDelIdx);
        }
    }
    LOG_DEBUG("delete node used_cnt[%d] free_cnt[%d]", LobbyHandle::m_pConnMng->GetUsedCount(), LobbyHandle::m_pConnMng->GetFreeCount());

    return 0;
}

int LobbyHandle::GetFlow()
{
    ++s_flow;
...
int LobbyHandle::GetHashKey(void *pvKey, int& iKeyLength)
{
    if (!pvKey)
    {
        iKeyLength = 0;
...
int LobbyHandle::SetHashKey(const void *pvKey, int iKeyLength)
{
