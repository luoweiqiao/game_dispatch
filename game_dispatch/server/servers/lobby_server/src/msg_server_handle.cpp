...
int  CHandleServerMsg::handle_msg_notify_change_account_data(NetworkObject* pNetObj, const uint8* pkt_buf, uint16 buf_len)
{
	...
	if (pPlayer != NULL)
	{
		cur_score = pPlayer->GetAccountValue(emACC_VALUE_COIN);
	}

	LOG_DEBUG("gameserver_updatescore satrt - uid:%d,pPlayer:%p,cur_score:%d,diamond:%lld,coin:%lld,score:%lld,ingot:%lld,cvalue:%lld,safecoin:%lld",uid, pPlayer, cur_score, diamond, coin, score, ingot, cvalue, safeCoin);
	...
	LOG_DEBUG("gameserver_updatescore   end - uid:%d,pPlayer:%p,cur_score:%d,diamond:%lld,coin:%lld,score:%lld,ingot:%lld,cvalue:%lld,safecoin:%lld", uid, pPlayer, cur_score, diamond, coin, score, ingot, cvalue, safeCoin);
