...
bool    CGameWarTable::CanEnterTable(CGamePlayer* pPlayer)
{
	...
	{
		LOG_DEBUG("error - roomid:%d,tableid:%d,uid:%d,m_mpLookers_size:%d", GetRoomID(), GetTableID(), pPlayer->GetUID(), m_mpLookers.size());
	...
	{
		LOG_DEBUG("error - roomid:%d,tableid:%d,uid:%d,m_mpLookers_size:%d,curScore:%lld,GetEnterMin:%lld",
			GetRoomID(), GetTableID(), pPlayer->GetUID(), m_mpLookers.size(), GetPlayerCurScore(pPlayer), GetEnterMin());

        return false;
    }

	LOG_DEBUG("roomid:%d,tableid:%d,uid:%d,m_mpLookers_size:%d", GetRoomID(), GetTableID(), pPlayer->GetUID(), m_mpLookers.size());
    ...
    case GER_NORMAL:		//常规结束
        {
            ...
            for(;it != m_mpLookers.end();++it)
            {
				...
				//LOG_DEBUG("send_player_game_end - roomid:%d,tableid:%d,uid:%d,score:%lld,robotWinScore:%d,playerFees:%d", GetRoomID(), GetTableID(), pPlayer->GetUID(), lUserScoreFree, robotWinScore, playerFees);
...
int64    CGameWarTable::CalcPlayerInfo(uint32 uid,int64 winScore,bool isBanker)
{
...
bool    CGameWarTable:: OnUserPlaceJetton(CGamePlayer* pPlayer, BYTE cbJettonArea, int64 lJettonScore)
{
	...
	LOG_DEBUG("table recv - roomid:%d,tableid:%d,uid:%d,cbJettonArea:%d,lJettonScore:%lld", GetRoomID(), GetTableID(), pPlayer->GetUID(), cbJettonArea, lJettonScore);
...
bool    CGameWarTable::DispatchTableCard()
{
	...
	if (!bAreaIsControl && !bIsFalgControl)
	...
	if (bAreaIsControl == false && bIsFalgControl == false && !bIsStockControl && needChangeCard)
	{
		bIsSysWinScore = SetSystemBrankerWinPlayerScore();
	}
	SetIsAllRobotOrPlayerJetton(IsAllRobotOrPlayerJetton()); // add by har
	LOG_DEBUG("roomid:%d,tableid:%d,bAreaIsControl:%d bIsFalgControl:%d,needChangeCard:%d,bIsSysWinScore:%d,bIsStockControl:%d,GetIsAllRobotOrPlayerJetton:%d",
		GetRoomID(), GetTableID(), bAreaIsControl, bIsFalgControl, needChangeCard, bIsSysWinScore, bIsStockControl, GetIsAllRobotOrPlayerJetton());

	for (uint16 i = 0; i < SHOW_CARD_COUNT; ++i)
	{
		LOG_DEBUG("DispatchTableCard card roomid:%d,tableid:%d,m_cbHandCardData:0x%02X 0x%02X 0x%02X",	GetRoomID(), GetTableID(),  m_cbTableCardArray[i][0], m_cbTableCardArray[i][1], m_cbTableCardArray[i][2]);
	}
...
int64 CGameWarTable::GetRobotJettonScore(CGamePlayer* pPlayer, uint8 area)
{
	...
	else if (lUserCurJetton >= 200000 && lUserCurJetton < 2000000)
	{
		...
		else if (g_RandGen.RandRatio(5300, PRO_DENO_10000))
		{
			lUserRealJetton = 10000;
		}
		else if (g_RandGen.RandRatio(1450, PRO_DENO_10000))
		{
			lUserRealJetton = 50000;
		}
		else
	...
	else if (lUserCurJetton >= 2000000)
	{
		...
		else if (g_RandGen.RandRatio(5350, PRO_DENO_10000))
		{
			lUserRealJetton = 50000;
		}
		else
