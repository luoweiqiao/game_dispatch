...
void 	CLobbyRobot::OnLoginOut(uint32 leaveparam)
{
...
bool  CLobbyRobot::NeedRecover()
{
    ...
    if(/*GetAccountValue(emACC_VALUE_SCORE) < GetLoginOutScore()
    ||*/ GetAccountValue(emACC_VALUE_COIN) < GetLoginOutCoin()){
        LOG_DEBUG("机器人%d破产了回收下线:%d--score:%lld--%lld,--coin:%lld--%lld",m_robotCfg.gameType,GetUID(),GetAccountValue(emACC_VALUE_SCORE),GetLoginOutScore(),GetAccountValue(emACC_VALUE_COIN),GetLoginOutCoin());
        return true;
    }
    if(getSysTime() > m_loginOutTime){
        //LOG_DEBUG("机器人在玩超过时间,下线休息:%d",GetUID());
        return true;
    }
    if(m_leaveRound <= 0) {
        LOG_DEBUG("机器人游戏局数满了,下线:%d--%d",GetUID(),GetGameType());
        return true;
    }
    if(m_needLoginOut){
        return true;
...
bool    CLobbyRobot::IsNeedStore(int64 &score,int64 &coin)
{
	...
	// 2倍一定会存钱，存钱的比例为身上钱的3分之1到7分之1，向下取整
	bool bIsStore = false;
...
bool    CLobbyRobot::IsNeedExchange(int64 &score,int64 &coin)
{
	...
	LOG_DEBUG("uid:%d,score:%lld,coin:%lld,richLvCoin:%lld,gameType:%d,curCoin:%lld,lProRandCoin:%lld",
		GetUID(), score, coin, richLvCoin, m_robotCfg.gameType, GetAccountValue(emACC_VALUE_COIN), lProRandCoin);
...
bool    CLobbyRobot::TidyCoin()
{
    ...
    if(IsNeedStore(score,coin))
    {
        if(!CGobalRobotMgr::Instance().StoreScoreCoin(this,score,coin)){
            LOG_ERROR("机器人存取财富币错误:%d",GetUID());
        }
    }
    // 兑换积分财富币
    if(IsNeedExchange(score, coin))
    {
        LOG_DEBUG("%d 需要兑换积分:%lld,财富币:%lld", GetUID(), score, coin);
        if(score > 0)
        {
...
bool    CLobbyRobot::CheckEnterToGameSvr()
{
    ...
    if(IsPlaying() && IsInLobby())
    {
        if(!NeedRecover())
        {
            ...
            if(svrid == 0)
			{
                LOG_DEBUG("robot enter game server id is 0.uid:%d",GetUID());
                return false;
            }
			if (m_robotCfg.gameType==net::GAME_CATE_EVERYCOLOR)
			{
				EnterEveryColorGameSvr(svrid);
			}
			else
			{
