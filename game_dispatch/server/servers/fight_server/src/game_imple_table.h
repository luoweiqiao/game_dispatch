...
//记录信息
struct fightGameRecord
{
	BYTE	wins[JETTON_INDEX_COUNT];//输赢标记
	BYTE	card;//桌面扑克
	fightGameRecord()
	{
		memset(this,0, sizeof(fightGameRecord));
	}
};
...
class CGameFightTable : public CGameTable
{
public:
	CGameFightTable(CGameRoom* pRoom,uint32 tableID,uint8 tableType);
    virtual ~CGameFightTable();
