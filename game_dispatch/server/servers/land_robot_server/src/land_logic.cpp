...
//获取类型
	BYTE CLandLogic::GetCardType(const BYTE cbCardData[], BYTE cbCardCount) {
		//简单牌型
		switch (cbCardCount) {
			case 0:    //空牌
			{
...
//排列扑克
	void CLandLogic::SortCardList(BYTE cbCardData[], BYTE cbCardCount, BYTE cbSortType) {
		//数目过虑
...
//混乱扑克
	void CLandLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount) {
		//混乱准备
	...
	void CLandLogic::ShuffleCard(BYTE cbCardData[], BYTE cbCardCount)
	{
		BYTE pos1 = 0,pos2 = 0;
		BYTE tmpCard = 0;
		for (uint8 i = 0; i < FULL_POKER_COUNT; ++i) {
			pos1 = g_RandGen.RandUInt() % cbCardCount;
			pos2 = g_RandGen.RandUInt() % cbCardCount;
			tmpCard = cbCardData[pos1];
			cbCardData[pos1] = cbCardData[pos2];
			cbCardData[pos2] = tmpCard;
		}
	}
//删除扑克
	bool CLandLogic::RemoveCardList(const BYTE cbRemoveCard[], BYTE cbRemoveCount, BYTE cbCardData[],
									BYTE cbCardCount) {
		//检验数据
		if(cbRemoveCount > cbCardCount)return false;

		//定义变量
		BYTE cbDeleteCount = 0, cbTempCardData[MAX_LAND_COUNT];
...
//逻辑数值
	BYTE CLandLogic::GetCardLogicValue(BYTE cbCardData) {
		//扑克属性
...
//对比扑克
	bool CLandLogic::CompareCard(const BYTE cbFirstCard[], const BYTE cbNextCard[], BYTE cbFirstCount,
								 BYTE cbNextCount) {
		//获取类型
    ...
    //出牌搜索
	bool CLandLogic::SearchOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult)
	{
		//设置结果
		memset(&OutCardResult,0,sizeof(OutCardResult));

		//构造扑克
		BYTE cbCardData[MAX_LAND_COUNT];
		BYTE cbCardCount=cbHandCardCount;
		memcpy(cbCardData,cbHandCardData,sizeof(BYTE)*cbHandCardCount);
				...
				if (cbTurnCardCount<=1)
				{
					for (BYTE i=0;i<AnalyseResult.cbBlockCount[0];i++)
					{
						...
						if (GetCardLogicValue(AnalyseResult.cbCardData[0][cbIndex])>cbLogicValue)
						{
							...
							memcpy(OutCardResult.cbResultCard,&AnalyseResult.cbCardData[0][cbIndex],sizeof(BYTE)*cbTurnCardCount);
				...
				if (cbTurnCardCount<=2)
				{
					for (BYTE i=0;i<AnalyseResult.cbBlockCount[1];i++)
					{
						...
						if (GetCardLogicValue(AnalyseResult.cbCardData[1][cbIndex])>cbLogicValue)
						{
							//设置结果
							OutCardResult.cbCardCount=cbTurnCardCount;
							memcpy(OutCardResult.cbResultCard,&AnalyseResult.cbCardData[1][cbIndex],sizeof(BYTE)*cbTurnCardCount);
				...
				if (cbTurnCardCount<=3)
				{
					for (BYTE i=0;i<AnalyseResult.cbBlockCount[2];i++)
					{
						...
						if (GetCardLogicValue(AnalyseResult.cbCardData[2][cbIndex])>cbLogicValue)
						{
							//设置结果
							OutCardResult.cbCardCount=cbTurnCardCount;
							memcpy(OutCardResult.cbResultCard,&AnalyseResult.cbCardData[2][cbIndex],sizeof(BYTE)*cbTurnCardCount);
				...
				for (BYTE i=cbTurnLineCount*3-1;i<cbCardCount;i++)
				{
					...
					for (BYTE j=(cbCardCount-i-1);j<(cbCardCount-2);j++)
					{
						...
						if (cbLineCount==cbTurnLineCount)
						{
							...
							//构造扑克
							BYTE cbLeftCardData[MAX_LAND_COUNT];
							BYTE cbLeftCount=cbCardCount-OutCardResult.cbCardCount;
							memcpy(cbLeftCardData,cbCardData,sizeof(BYTE)*cbCardCount);
							RemoveCardList(OutCardResult.cbResultCard,OutCardResult.cbCardCount,cbLeftCardData,cbCardCount);
...
//分析扑克
	void CLandLogic::AnalysebCardData(const BYTE cbCardData[], BYTE cbCardCount, tagAnalyseResult &AnalyseResult) {
		//设置结果
...
//分析分布
	void CLandLogic::AnalysebDistributing(const BYTE cbCardData[], BYTE cbCardCount, tagDistributing &Distributing) {
		//设置变量
    ...
    //设置扑克
    void CLandLogic::SetUserCard(WORD wChairID, BYTE cbCardData[], BYTE cbCardCount)
    {
    ...
    //设置底牌
    void CLandLogic::SetBackCard(WORD wChairID, BYTE cbBackCardData[], BYTE cbCardCount)
    {
    ...
    //设置庄家
    void CLandLogic::SetBanker(WORD wBanker) 
    {
    	m_wBankerUser = wBanker ;
    }
    //叫牌扑克
    void CLandLogic::SetLandScoreCardData(WORD wChairID,BYTE cbCardData[], BYTE cbCardCount) 
    {
    	if(cbCardCount!=MAX_LAND_COUNT) return ;

    	memcpy(m_cbLandScoreCardData[wChairID], cbCardData, cbCardCount*sizeof(BYTE));
    	//排列扑克
    	SortCardList(m_cbLandScoreCardData[wChairID],cbCardCount,ST_ORDER);
    }
    //删除扑克
    void CLandLogic::RemoveUserCardData(WORD wChairID, BYTE cbRemoveCardData[], BYTE cbRemoveCardCount) 
    {
    	bool bSuccess = RemoveCardList(cbRemoveCardData, cbRemoveCardCount, m_cbAllCardData[wChairID], m_cbUserCardCount[wChairID]) ;
    	if(bSuccess){
    	    m_cbUserCardCount[wChairID] -= cbRemoveCardCount;
    	}
    }
    //组合算法
	void CLandLogic::Combination(BYTE cbCombineCardData[], BYTE cbResComLen,  BYTE cbResultCardData[254][5], BYTE &cbResCardLen,BYTE cbSrcCardData[] , BYTE cbCombineLen1, BYTE cbSrcLen, const BYTE cbCombineLen2)
    {
	...
	//排列算法
	void CLandLogic::Permutation(BYTE *list, int m, int n, BYTE result[][4], BYTE &len)
	{
    	int j,temp; 
    	if(m == n){ 
    		for(j = 0; j < n; j++) 
	...
	//分析炸弹
	void CLandLogic::GetAllBomCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbBomCardData[], BYTE &cbBomCardCount)
	{
        BYTE cbTmpCardData[MAX_LAND_COUNT] ;
	...
	//分析顺子
	void CLandLogic::GetAllLineCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbLineCardData[], BYTE &cbLineCardCount)
	{
    	BYTE cbTmpCard[MAX_LAND_COUNT] ;
	...
	//分析三条
	void CLandLogic::GetAllThreeCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbThreeCardData[], BYTE &cbThreeCardCount)
	{
        BYTE cbTmpCardData[MAX_LAND_COUNT] ;
    	memcpy(cbTmpCardData, cbHandCardData, cbHandCardCount) ;
	...
	//分析对子
	void CLandLogic::GetAllDoubleCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbDoubleCardData[], BYTE &cbDoubleCardCount)
	{
        BYTE cbTmpCardData[MAX_LAND_COUNT] ;
    	memcpy(cbTmpCardData, cbHandCardData, cbHandCardCount) ;
	...
	//分析单牌
	void CLandLogic::GetAllSingleCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbSingleCardData[], BYTE &cbSingleCardCount)
	{
        cbSingleCardCount =0 ;
    	BYTE cbTmpCardData[MAX_LAND_COUNT] ;
    	memcpy(cbTmpCardData, cbHandCardData, cbHandCardCount) ;
	...
	//分析牌牌（先出牌调用）
    void CLandLogic::AnalyseOutCardType(BYTE const cbHandCardData[], BYTE const cbHandCardCount, tagOutCardTypeResult CardTypeResult[12+1])
	{
        memset(CardTypeResult,0,sizeof(CardTypeResult[0])*12) ;
    	BYTE cbTmpCardData[MAX_LAND_COUNT] ;
    	//保留扑克，防止分析时改变扑克
    	BYTE cbReserveCardData[MAX_LAND_COUNT] ;
    	...
    	//对牌类型
    	{
    		BYTE cbDoubleCardData[MAX_LAND_COUNT] ;
    	...
    	//三条类型
    	{
    		BYTE cbThreeCardData[MAX_LAND_COUNT];
    	...
    	//炸弹类型
    	{
    		BYTE cbFourCardData[MAX_LAND_COUNT];
    		...
    		while (cbLeftCardCount>=5 && bFindSingleLine)
    		{
    			...
    			if(cbSingleLineCount>=5)
    			{
    				...
    				while (cbSingleLineCount-cbStart>=5)
    				{
    					...
    					memcpy(CardTypeResult[CT_SINGLE_LINE].cbCardData[Index], cbSingleLineCard+cbStart, sizeof(BYTE)*(cbThisLineCount));
    	...
    	//对连类型
    	{
    		...
    		while (cbLeftCardCount>=6 && bFindDoubleLine)
    		{
    			...
    			if(cbDoubleLineCount>=6)
    			{
    				...
    				memcpy(CardTypeResult[CT_DOUBLE_LINE].cbCardData[Index], cbDoubleLineCard, sizeof(BYTE)*cbDoubleLineCount);
    	...
    	//三带一单
    	{
    		...
    		BYTE cbHandThreeCard[MAX_LAND_COUNT];
    		BYTE cbHandThreeCount=0 ;

    		//移除炸弹
    		BYTE cbAllBomCardData[MAX_LAND_COUNT] ;
...

    		{
    			BYTE Index ;
    			//去掉三条
    			BYTE cbRemainCardData[MAX_LAND_COUNT] ;
    			...
    			RemoveCardList(cbHandThreeCard, cbHandThreeCount, cbRemainCardData, cbHandCardCount-cbAllBomCardCount) ;
    			//三条带一张
    		...
    		while (cbLeftThreeCardCount>=6 && bFindThreeLine)
    		{
    			...
    			BYTE cbThreeLineCard[MAX_LAND_COUNT];
    			...
    			if(cbThreeLineCardCount>3)
    			{
    				...
    				BYTE cbRemainCard[MAX_LAND_COUNT];
    				BYTE cbRemainCardCount=cbHandCardCount-cbAllBomCardCount-cbHandThreeCount ;


    				//移除三条（还应该移除炸弹王等）
    				memcpy(cbRemainCard, cbTmpCardData, (cbHandCardCount-cbAllBomCardCount)*sizeof(BYTE));
    				RemoveCardList(cbHandThreeCard, cbHandThreeCount, cbRemainCard, cbHandCardCount-cbAllBomCardCount) ;
    				...
    				for(BYTE start=0; start<cbThreeLineCardCount-3; start+=3)
    				{
    					...
    					for(BYTE i=0; i<cbComResLen; ++i)
    					{
    						...
    						CardTypeResult[CT_THREE_TAKE_ONE].cbCardType = CT_THREE_TAKE_ONE ;
    						//保存三条
    						memcpy(CardTypeResult[CT_THREE_TAKE_ONE].cbCardData[Index], cbThreeLineCard+start, sizeof(BYTE)*cbThisTreeLineCardCount);
    						//保存单牌
    						memcpy(CardTypeResult[CT_THREE_TAKE_ONE].cbCardData[Index]+cbThisTreeLineCardCount, cbComResCard[i], cbSingleCardCount) ;
    	...
    	//三带一对
    	{
    		...
    		BYTE cbHandThreeCard[MAX_LAND_COUNT];
    		BYTE cbHandThreeCount=0 ;
    		BYTE cbRemainCarData[MAX_LAND_COUNT] ;
    		...
    		//抽取对牌
    		BYTE cbAllDoubleCardData[MAX_LAND_COUNT] ;
    			...
    			if(cbThreeLineCardCount>3)
    			{
    				...
    				for(BYTE start=0; start<cbThreeLineCardCount-3; start+=3)
    				{
    					...
    					for(BYTE i=0; i<cbComResLen; ++i)
    					{
    						...
    						CardTypeResult[CT_THREE_TAKE_TWO].cbCardType = CT_THREE_TAKE_TWO ;
    						//保存三条
    						memcpy(CardTypeResult[CT_THREE_TAKE_TWO].cbCardData[Index], cbThreeLineCard+start, sizeof(BYTE)*cbThisTreeLineCardCount);
    						//保存对牌
    	...
    	/*{
        	...
        	BYTE cbHandAllFourCardData[MAX_LAND_COUNT] ;
	...
	//分析牌型（后出牌调用）
	void CLandLogic::AnalyseOutCardType(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE const cbTurnCardData[], BYTE const cbTurnCardCount, tagOutCardTypeResult CardTypeResult[12+1])	
	{
        memset(CardTypeResult,0, sizeof(CardTypeResult[0])*12) ;
    	//数据校验
    	...
    	if(cbTurnCardType!=CT_MISSILE_CARD && cbTurnCardType!=CT_BOMB_CARD)
    	{
    		...
    		if(cbHandCardCount>=2 && 0x4F==cbTmpCard[0] && 0x4E==cbTmpCard[1])
    		{
    			...
    			BYTE cbBomCardData[MAX_LAND_COUNT];
    	...
    	case CT_THREE_TAKE_ONE://三带一单
    		{
    			BYTE cbTurnThreeCard[MAX_LAND_COUNT]; 
					...
					for (BYTE i = 0; i < (cbHandCardCount - cbAllBomCardCount); ++i)
					{
    					...
    					for(BYTE i=0; i<cbHandThreeCount; i+=3)
    					{
    						...
    						if(cbThreeLineCardCount==cbTurnThreeCount && GetCardLogicValue(cbThreeLineCard[0])>GetCardLogicValue(cbTurnThreeCard[0]))
    						{
    							...
    							RemoveCardList(cbThreeLineCard, cbTurnThreeCount, cbRemainCard, (cbHandCardCount-cbAllBomCardCount)) ;
    							...
    							for(BYTE i=0; i<cbComResLen; ++i)
    							{
    								...
    								CardTypeResult[CT_THREE_TAKE_ONE].cbCardType = CT_THREE_TAKE_ONE;
    								//保存三条
    								memcpy(CardTypeResult[CT_THREE_TAKE_ONE].cbCardData[Index], cbThreeLineCard, sizeof(BYTE)*cbTurnThreeCount);
    								//保存单牌
    								memcpy(CardTypeResult[CT_THREE_TAKE_ONE].cbCardData[Index]+cbTurnThreeCount, cbComResCard[i], cbSingleCardCount) ;
    	...
    	case CT_THREE_TAKE_TWO://三带一对
    		{
				...
				if (cbTurnThreeCount > 3)
				{
					for (BYTE i = 0; i < cbHandCardCount; ++i)
					{
						if (GetCardLogicValue(cbTmpCard[i]) < 15) { cbFirstCard = i; break; }
					}
				}
    				...
    				for(BYTE i=0; i<cbHandThreeCount; i+=3)
    				{
    					...
    					if(cbThreeLineCardCount==cbTurnThreeCount && GetCardLogicValue(cbThreeLineCard[0])>GetCardLogicValue(cbTurnThreeCard[0]))
    					{
    						...
    						for(BYTE i=0; i<cbComResLen; ++i)
    						{
    							...
    							CardTypeResult[CT_THREE_TAKE_TWO].cbCardType = CT_THREE_TAKE_TWO ;
    							//保存三条
    							memcpy(CardTypeResult[CT_THREE_TAKE_TWO].cbCardData[Index], cbThreeLineCard, sizeof(BYTE)*cbTurnThreeCount);
    							//保存对牌
    							for(BYTE j=0, k=0; j<cbDoubleCardCount; ++j, k+=2)
    							{
    	...
    	case CT_FOUR_TAKE_ONE://四带两单
    		{
    			...
    			BYTE cbHandAllFourCardData[MAX_LAND_COUNT];
    			BYTE cbHandAllFourCardCount=0;
    			BYTE cbTurnAllFourCardData[MAX_LAND_COUNT];
    			...
    			BYTE cbCanOutFourCardData[MAX_LAND_COUNT];
	...
	//单牌个数
	BYTE CLandLogic::AnalyseSinleCardCount(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE const cbWantOutCardData[], BYTE const cbWantOutCardCount, BYTE cbSingleCardData[])
	{
        BYTE cbRemainCard[MAX_LAND_COUNT] ;
    	BYTE cbRemainCardCount=0 ;
    	...
    	if(cbWantOutCardCount!=0) RemoveCardList(cbWantOutCardData, cbWantOutCardCount, cbRemainCard, cbHandCardCount) ;
    	cbRemainCardCount = cbHandCardCount-cbWantOutCardCount ;

    	//函数指针
    	typedef void (CLandLogic::*pGetAllCardFun)(BYTE const [], BYTE const , BYTE[], BYTE &); 
    	...
    	ASSERT(cbLen==24) ;
    	if(cbLen!=24) return MAX_LAND_COUNT ;

    	//单牌数目
    	BYTE cbMinSingleCardCount = MAX_LAND_COUNT ;
    	//计算最小值
    	for(BYTE i=0; i<24; ++i)
    	{
    		//保留数据
    		...
    		for(BYTE j=0; j<4; ++j)
    		{
    			...
    			//校验下标
    			ASSERT(Index<4) ;
    			if(Index>=4) return MAX_LAND_COUNT ;

    			pGetAllCardFun pTmpGetAllCardFun = GetAllCardFunArray[Index] ;

    			//提取扑克
    			BYTE cbGetCardData[MAX_LAND_COUNT] ;
    			BYTE cbGetCardCount=0 ;
    			//成员函数
    			((*this).*pTmpGetAllCardFun)(cbTmpCardData, cbTmpCardCount, cbGetCardData, cbGetCardCount) ;

    			//删除扑克
    			if(cbGetCardCount!=0) RemoveCardList(cbGetCardData, cbGetCardCount, cbTmpCardData, cbTmpCardCount) ;
    			cbTmpCardCount -= cbGetCardCount ;
    		}

    		//计算单牌
    		BYTE cbSingleCard[MAX_LAND_COUNT] ;
    ...
    //地主出牌（先出牌）
    void CLandLogic::BankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, tagOutCardResult & OutCardResult) 
    {
    	//零下标没用
    	...
    	BYTE cbLineCard[MAX_LAND_COUNT] ;
    	BYTE cbThreeLineCard[MAX_LAND_COUNT] ;
    	BYTE cbDoubleLineCard[MAX_LAND_COUNT] ;
    	...
    	WORD wUndersideOfBanker = (m_wBankerUser+1)%GAME_LAND_PLAYER ;	    //地主下家
    	WORD wUpsideOfBanker = (wUndersideOfBanker+1)%GAME_LAND_PLAYER ;	//地主上家
    	...
    	{
    		{
    			...
    			BYTE cbMinValue=MAX_LAND_COUNT ; 
    			BYTE cbMinSingleCountInFour=MAX_LAND_COUNT ;
    			...
    			for(BYTE cbCardType=CT_DOUBLE; cbCardType<CT_BOMB_CARD; ++cbCardType)
    			{
    				...
    				BYTE cbThisHandCardCount = MAX_LAND_COUNT ;
    ...
    //地主出牌（后出牌）
    void CLandLogic::BankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wOutCardUser, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult) 
    {
    	//初始变量
    	memset(&OutCardResult,0,sizeof(OutCardResult)) ;
    	...
    	WORD wUndersideUser = (m_wBankerUser+1)%GAME_LAND_PLAYER ;
    	WORD wUpsideUser = (wUndersideUser+1)%GAME_LAND_PLAYER ;
    	...
    	if(CanOutCard.cbCardTypeCount>0)
    	{
    		...
    		BYTE cbMinSingleCount=MAX_LAND_COUNT ;
    		BYTE Index=0 ;
    ...
    //地主上家（先出牌）
    void CLandLogic::UpsideOfBankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wMeChairID, tagOutCardResult & OutCardResult)
    {
    	//零下标没用
    	tagOutCardTypeResult CardTypeResult[12+1] ;
    	memset(CardTypeResult,0, sizeof(CardTypeResult)) ;
		...
		else
		{
    		...
    		//分析地主单牌
    		BYTE cbBankerSingleCardData[MAX_LAND_COUNT] ;
    ...
    //地主上家（后出牌）
    void CLandLogic::UpsideOfBankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wOutCardUser, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult) 
    {
    		...
    		(wOutCardUser==m_wBankerUser)))//地主出牌
    	{
    		...
    		for(BYTE i=0; i<CardTypeResult[cbOutCardType].cbCardTypeCount; ++i)
    		{
    			...
    			if(  (BankerOutCardTypeResult[cbOutCardType].cbCardTypeCount>0 && GetCardLogicValue(CardTypeResult[cbOutCardType].cbCardData[i][0])>=GetCardLogicValue(BankerOutCardTypeResult[cbOutCardType].cbCardData[0][0]))
                  || (BankerOutCardTypeResult[cbOutCardType].cbCardTypeCount==0 && cbTmpSingleCardCount<=cbThisOutTypeMinSingleCount)  )
    			{
    				cbBestIndex = i ;
    	...
    	if(bFriendOut)
    	{
    		if(CanOutCard.cbCardTypeCount>0)
    		{
    			...
    			else if(cbHandCardCount<=5)
    			{
					...
					for (BYTE i = 0; i < 4; ++i)
					{
						if (cbMinSingleCardCount[i]<MAX_LAND_COUNT && cbMinSingleCardCount[i] <= cbMinSingleCountInFour && cbMinLogicCard>GetCardLogicValue(CanOutCard.cbCardData[cbIndex[i]][0]) &&
    ...
    //地主下家（先出牌）
    void CLandLogic::UndersideOfBankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wMeChairID,tagOutCardResult & OutCardResult) 
    {
    	//零下标没用
    	tagOutCardTypeResult CardTypeResult[12+1] ;
    	memset(CardTypeResult,0, sizeof(CardTypeResult)) ;
    ...
    //地主下家（后出牌）
    void CLandLogic::UndersideOfBankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wOutCardUser, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult)
    {
    	//初始变量
    	memset(&OutCardResult,0, sizeof(OutCardResult)) ;
    	...
    	else
    	{
    		if(CanOutCard.cbCardTypeCount>0)
    		{
    			...
    			//当朋友不能拦截地主时
    			WORD wMeChairID = (m_wBankerUser+1)%GAME_LAND_PLAYER ;
    			WORD wFriendID = (wMeChairID+1)%GAME_LAND_PLAYER ;
    ...
    //出牌搜索
    bool CLandLogic::SearchOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, WORD wOutCardUser, WORD wMeChairID, tagOutCardResult & OutCardResult)
    {
    	//玩家判断
    	WORD wUndersideOfBanker = (m_wBankerUser+1)%GAME_LAND_PLAYER ;	    //地主下家
    	WORD wUpsideOfBanker = (wUndersideOfBanker+1)%GAME_LAND_PLAYER ;    //地主上家
    ...
    //叫分判断
    BYTE CLandLogic::LandScore(WORD wMeChairID, BYTE cbCurrentLandScore)
    {
    	//大牌数目
    	...
    	//单牌个数
    	BYTE cbSingleCardCount=AnalyseSinleCardCount(m_cbLandScoreCardData[wMeChairID], sizeof(m_cbLandScoreCardData[wMeChairID]), NULL, 0) ;
    	...
    	//其他单牌
    	BYTE cbMinSingleCardCount=MAX_LAND_COUNT ;
    	for(WORD wChairID=0 , i=0; wChairID<GAME_LAND_PLAYER; ++wChairID)
    	{
	...
	BYTE CLandLogic::GetCardIndex(BYTE cbCardData)
	{
		BYTE cbIndex = 0;
		if (cbCardData == 0x4E)
		{
		...
		{
			cbIndex = GetCardValue(cbCardData);
		}
		if (cbIndex >= MAX_SIG_CARD_COUNT)
		{
			cbIndex = 0;
		}
		return cbIndex;
