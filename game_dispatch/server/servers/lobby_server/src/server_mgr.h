...
public:
	...
	void	GetGameTypeSvrID(uint32 gametype, vector<uint16> & vecServerID);
    ...
    // 控制玩家
	void ChangeContorlPlayer(uint32 gametype,uint32 roomid, uint32 uid, uint32 operatetype, uint32 gamecount);
	void ChangeContorlMultiPlayer(uint32 gametype, uint32 roomid, uint32 uid, uint32 operatetype, uint32 gamecount, uint64 gametime, int64 totalscore);

	// 
	void StopSnatchCoin(uint32 gametype, uint32 roomid, uint32 stop);

	void SetSvrStatus(uint16 svrID, uint8 status);
	...
	void UpdateRobotOnlineCfg(stRobotOnlineCfg & refCfg);

	void DeleteRobotOnlineCfg(int gametype, int batchid);
