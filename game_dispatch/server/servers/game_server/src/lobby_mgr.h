...
public:
	...
	bool    SendMsg2Lobby(const google::protobuf::Message* msg, uint16 msg_type, uint16 svrid);  
	void    SendMsg2AllLobby(const google::protobuf::Message* msg, uint16 msg_type);
	...
	void    ConnectNewLobby(uint32 svrid);
	void	NotifyLobbyRetired(uint32 svrid);
	stLobbyServer*  GetLobbySvrBySvrID(uint16 svrid);
    stLobbyServer * GetLobbySvrBypNetObj(NetworkObject* pNetObj);
private:
