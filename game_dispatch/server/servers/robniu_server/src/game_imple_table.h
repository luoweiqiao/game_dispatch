...
// 游戏桌子
class CGameRobniuTable : public CGameTable
{
public:
	CGameRobniuTable(CGameRoom* pRoom,uint32 tableID,uint8 tableType);
    virtual ~CGameRobniuTable();
...
protected:
    ...
    void    CalculateScore();
	// 结算玩家分数  add by har
	// cbHandCardData : 要结算的牌组
	// return : 返回玩家总赢分
	int64 CalculatePlayerScore(uint8 cbHandCardData[GAME_PLAYER][ROBNIU_CARD_NUM]);
...
protected:
	...
	bool SetPlayerBrankerCanWinScore(int64 maxscore);

	bool SetPlayerNotBrankerCanWinScore(CGamePlayer* pPlayer, int64 maxscore);

	bool SetControlPlayerMaxWinScore(CGamePlayer* pPlayer, int64 maxscore);

    // 活跃福利控制
    //bool ActiveWelfareCtrl();

    // 控制活跃福利的必赢玩家
    bool SetControlPlayerWinForAW(uint32 control_uid, int64 max_win);

    // 活跃福利控制---获取当前控制玩家的输赢值
    int64 GetAwWinScore(uint32 aw_uid);

    int32 GetAwPlayerChairId(uint32 aw_uid);
...
protected:
	...
	//扑克变量
protected:
	BYTE							m_cbHandCardData[GAME_PLAYER][ROBNIU_CARD_NUM];       //桌面扑克
	BYTE							m_cbChangeCardData[GAME_PLAYER][ROBNIU_CARD_NUM];       //桌面扑克
...
protected:
    CRobNiuLogic                    m_gameLogic;                            //游戏逻辑
    CCooling                        m_coolRobot;                            //机器人CD
	uint32                          m_robotWinPro;							//机器人赢概率
	int64                           m_stockChange;                          //库存改变值 add by har
};
