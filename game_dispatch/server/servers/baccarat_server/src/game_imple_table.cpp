...
bool    CGameBaccaratTable::DispatchTableCard()
{
	...
	if (!bAreaIsControl && !bBrankerIsControl && !bIsControlPlayerIsJetton && tmpJackpotScore.iUserJackpotControl==1 && tmpJackpotScore.lCurPollScore>tmpJackpotScore.lMaxPollScore && bIsSysLostPro) // 吐币
	{
	...
	if (!bAreaIsControl && !bBrankerIsControl && !bIsControlPlayerIsJetton && tmpJackpotScore.iUserJackpotControl == 1 && tmpJackpotScore.lCurPollScore<tmpJackpotScore.lMinPollScore && bIsSysWinPro) // 吃币
	{
		bIsPoolScoreControl = true;
		if (bBrankerIsRobot)
		{
...
bool    CGameBaccaratTable::DispatchTableCardBrankerIsPlayer()
{
	...
	{// 100次没有随机到就放弃
		...
		memset(cbWinArea, 0, sizeof(cbWinArea));
		cbBankerCount = DeduceWinner(cbWinArea); // 补牌后点数可能会改变，必须重新获取庄家点数，否则后面计算赢分可能会不正确！  modify by har
