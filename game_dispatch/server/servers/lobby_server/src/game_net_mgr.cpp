...
{
    LOG_ERROR("client handle heart timeout this:%p uid:%d", this, GetUID());
    //! 心跳超时
    DestroyObj();
}
...
int ClientHandle::DeleteNode(unsigned int Key)
{
    ...
    LOG_DEBUG("delete node used_cnt[%d] free_cnt[%d]", ClientHandle::m_pConnMng->GetUsedCount(), ClientHandle::m_pConnMng->GetFreeCount());
