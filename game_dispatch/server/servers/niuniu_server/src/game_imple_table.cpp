...
namespace
{
	...
	const static uint32 s_GameEndTime		 = 14 * 1000; // 游戏结束时间
...
bool    CGameNiuniuTable::OnGameEnd(uint16 chairID,uint8 reason)
{
	...
	case GER_NORMAL:
		{
            //CalculateScore(); // delete by har
		    //int64 playerFees;
		    //int64 robotAllWinScore = -GetBankerAndPlayerWinScore(m_lWinScore, m_cbBrankerSettleAccountsType, playerFees); // add by har
            ...
            //    if (pTmp != NULL && pTmp->GetUID()== m_aw_ctrl_uid)
            //    {
            //        aw_ctrl_chairid = i;
            //        break;
            //    }               
            ...
            //{
            //    int64 curr_win = m_lWinScore[m_aw_ctrl_uid];
...
{	
	...
	//}
	//if (m_bIsProgressControlPalyer == false)
	//{
	...
	LOG_DEBUG("prob - roomid:%d,tableid:%d,ControlPalyer:%d,NoviceWelfareCtrl:%d,uids:%s,status:%s",
		GetRoomID(), GetTableID(), m_bIsProgressControlPalyer,m_bIsNoviceWelfareCtrl, strAllPlayerUid.c_str(), strAllPlayerStatus.c_str());
...
bool    CGameNiuniuTable::OnMasterUserOper(CGamePlayer* pPlayer, vector<BYTE> vecChairID, vector<BYTE> vecCardData)
{
	...
	//bool bIsGameStateTrue = false;
	//bool bIsAllRobot = true;
	//if (bIsSuccessState == false)
	//{
	//	// 如果状态不对 改的全是机器人可以修改
	//	for (uint32 i = 0; i < vecChairID.size(); i++)
	//	{
	//		uint32 uChairID = vecChairID[i];
	//		CGamePlayer * pTagPlayer = GetPlayer(uChairID);
	//		if (pTagPlayer !=NULL)
	//		{
	//			if (pTagPlayer->IsRobot() == false)
	//			{
	//				bIsAllRobot = false;
	//			}
	//		}
...
bool CGameNiuniuTable::HaveWelfareNovicePlayer(uint32 & noviceuid, uint32 & posrmb)
{
	int count = 0;
	noviceuid = 0;
	posrmb = 0;
...
//	uint32 noviceuid = 0;
//	uint32 posrmb = 0;
//	bool bHaveNovice = HaveWelfareNovicePlayer(noviceuid, posrmb);
//
//	int isnewnowe = 0;
...
//		bIsHitWelfarePro = g_RandGen.RandRatio(NewPlayerWelfareValue.welfarepro, PRO_DENO_10000);
//		if (bIsHitWelfarePro)
//		{
...
//
//
//	LOG_DEBUG("dos_wel_ctrl - roomid:%d,tableid:%d,isnewnowe:%d, newmaxjetton:%lld, newsmaxwin:%lld,lNoviceWinScore:%lld, IsNoviceWelfareCtrl:%d, noviceuid:%d,posrmb:%d, bIsOverJetton:%d, bHaveNovice:%d,ChessWelfare:%d,welfarepro:%d,bIsHitWelfarePro:%d",
//		GetRoomID(), GetTableID(), isnewnowe, newmaxjetton, newsmaxwin, lNoviceWinScore, bIsNoviceWelfareCtrl, noviceuid, posrmb, bIsOverJetton, bHaveNovice, GetChessWelfare(), NewPlayerWelfareValue.welfarepro, bIsHitWelfarePro);
//
//
...
void CGameNiuniuTable::SetCardDataControl()
{
    ...
    if (!m_bIsProgressControlPalyer && !m_bIsNoviceWelfareCtrl && !m_bIsMasterUserOper && !bIsLuckyCtrl)
    {
	...
	if (m_bIsProgressControlPalyer == false && !m_bIsMasterUserOper && m_bIsNoviceWelfareCtrl == false && bIsAWControl == false && bIsRobotCanWin == true && !bIsLuckyCtrl)
	{
	...
	// add by har
	m_isNeedCheckStock = false;
	if (!m_bIsProgressControlPalyer && !m_bIsMasterUserOper && !m_bIsNoviceWelfareCtrl && !bIsAWControl && !bIsRobotCanWin && !bIsLuckyCtrl && !bIsRobotCanWin)
		m_isNeedCheckStock = true; // add by har end

	LOG_DEBUG("roomid:%d,tableid:%d,chessid:%s,ControlPalyer:%d,m_bIsMasterUserOper:%d,NoviceWelfareCtrl:%d,bIsRobotWinScore:%d,m_robotWinPro:%d bIsAWControl:%d bIsLuckyCtrl:%d,m_isNeedCheckStock:%d",
		GetRoomID(), GetTableID(), GetChessID().c_str(), m_bIsProgressControlPalyer, m_bIsMasterUserOper, m_bIsNoviceWelfareCtrl, bIsRobotWinScore, m_robotWinPro, bIsAWControl, bIsLuckyCtrl, m_isNeedCheckStock);
...
bool    CGameNiuniuTable::SetRobotWinCard()
{
	...
	for (uint8 i = 0; i<m_vecPlayers.size(); ++i)
	{
		...
		if (m_vecPlayers[i].pPlayer->IsRobot()==true)
		{
			robotcount++;
		}
		else
	...
	CGamePlayer * pBrankerPlayer = GetPlayer(m_wBankerUser);
	if (pBrankerPlayer != NULL && pBrankerPlayer->IsRobot() == false)
	{
		return SetControlPalyerLost(pBrankerPlayer->GetUID());
	}
