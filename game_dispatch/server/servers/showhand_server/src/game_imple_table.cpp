    ...
    case GER_NO_PLAYER:		//没有玩家
        {
            ...
            for(WORD i=0;i<GAME_PLAYER;i++)
            {
				...
				if (pGamePlayer != NULL && !pGamePlayer->IsRobot())
					playerAllWinScore += ScoreInfoArray[i]; // add by har
            }
...
void CGameShowHandTable::DispatchCard()
{
	...
	/*if (!bIsFalgControl && !bIsNoviceWelfareCtrl)
	{
		bIsNRWControl = NewRegisterWelfareCtrl();
	}*/
	
	bool bIsWinMaxScorePlayerLost = false;
	if (!bIsFalgControl && !bIsNRWControl && !bIsNoviceWelfareCtrl && !bIsLuckyCtrl)
	{
		//玩家最多能赢
	...
	if (!bIsFalgControl && !bIsNoviceWelfareCtrl && !bIsNRWControl && !bIsWinMaxScorePlayerLost && tmpJackpotScore.iUserJackpotControl == 1 && !bIsLuckyCtrl)
	{
	...
	if (needChangeCard && !bIsFalgControl && !bIsNoviceWelfareCtrl && !bIsNRWControl && !bIsWinMaxScorePlayerLost && !bIsPoolScoreControl && !bIsLuckyCtrl)
	{
		bIsSetRobotWin = true;
	...
	LOG_DEBUG("roomid:%d,tableid:%d,bIsFalgControl:%d,bIsLuckyCtrl:%d,bIsAWCtrl:%d,bIsNRWControl:%d,bIsNoviceWelfareCtrl:%d,bIsWinMaxScorePlayerLost:%d,bIsPoolScoreControl:%d,iUserJackpotControl:%d,needChangeCard:%d,bIsSetRobotWin:%d,bIsStockControl:%d,GetIsAllRobotOrPlayerJetton:%d,m_cbHandCardData:0x%02X 0x%02X 0x%02X 0x%02X 0x%02X - 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
		...
		m_cbHandCardData[0][0], m_cbHandCardData[0][1], m_cbHandCardData[0][2], m_cbHandCardData[0][3], m_cbHandCardData[0][4], m_cbHandCardData[1][0], m_cbHandCardData[1][1], m_cbHandCardData[1][2], m_cbHandCardData[1][3], m_cbHandCardData[1][4] );
...
bool    CGameShowHandTable::SetControlPalyerLost_card_type(uint32 control_uid)
{
	...
	LOG_DEBUG("changer is no find - roomid:%d,tableid:%d,minChairID:%d,control_uid:%d", GetRoomID(), GetTableID(), minChairID, control_uid);
	return false;
}
