...
typedef struct
{
	...
	uint32	CurBetScore;//当前押注的钱ID
	uint32 	FreeTimes; //当前免费的次数
	int64   Free_winscore;//免费赢钱和
	bool bIsMasterCtrl;
}FruitUserData;
...
public:
	...
	// out_winscore out: 玩家赢分
	bool SetControlPalyerWin(CGamePlayer *pPlayer, uint32 nline, uint32 nbet, uint32 mGamePic[], int64 &out_winscore);
