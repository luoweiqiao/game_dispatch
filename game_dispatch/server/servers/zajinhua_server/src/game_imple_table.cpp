...
bool    CGameZajinhuaTable::CanEnterTable(CGamePlayer* pPlayer)
{
	...
	LOG_DEBUG("uid:%d,roomid:%d,tableid:%d,bIsNewRegisterWelfare:,bIsNoviceWelfare:",
	...
	case GER_NO_PLAYER:		//没有玩家
		{
			...
			if (m_pHostRoom != NULL && m_cbPlayIsRobot[wWinner]== emPLAYER_IS_ROBOT)
			{
				...
				{
					LOG_DEBUG("CGameZajinhuaTable::OnGameEnd2  - roomid:%d,tableid:%d,lUpdatePollScore:%lld", m_pHostRoom->GetRoomID(), GetTableID(), lUpdatePollScore);
				}
...
void CGameZajinhuaTable::DispatchCard()
{
	...
	bool bIsNoviceWelfareCtrl = false;
	/*if (bIsNRWControl ==false && bIsFalgControl == false)
	{
    ...
    /*if(!bIsNRWControl && bIsNoviceWelfareCtrl == false)
    {
	...
	if (!bIsControlPlayerIsReady && !bIsAWControl && !bIsNRWControl&& bIsNoviceWelfareCtrl == false && tmpJackpotScore.iUserJackpotControl == 1 && !bLuckyCtrl)
	{
	...
	if (!bIsControlPlayerIsReady && !bIsAWControl && !bIsNRWControl && !bIsNoviceWelfareCtrl && !bLuckyCtrl && !bIsPoolScoreControl) {
		isStockCtrl = SetStockWinLose();
	}
	SetIsAllRobotOrPlayerJetton(IsAllRobotOrPlayerJetton()); // add by har
	LOG_DEBUG("conter game card - roomid:%d,tableid:%d,bIsControlPlayerIsReady:%d,bIsAWControl:%d,bIsNRWControl:%d,bIsNoviceWelfareCtrl:%d,bIsSysWinPro:%d,bIsSysLostPro:%d,bIsPoolScoreControl:%d,lCurPollScore:%lld,lMaxPollScore:%lld,lMinPollScore:%lld,bIsFalgControl:%d,control_uid:%d,game_count:%d,control_type:%d,bLuckyCtrl:%d,isStockCtrl:%d,GetIsAllRobotOrPlayerJetton:%d",
		GetRoomID(), GetTableID(), bIsControlPlayerIsReady, bIsAWControl, bIsNRWControl, bIsNoviceWelfareCtrl, bIsSysWinPro,
		bIsSysLostPro, bIsPoolScoreControl, tmpJackpotScore.lCurPollScore, tmpJackpotScore.lMaxPollScore,tmpJackpotScore.lMinPollScore, bIsFalgControl, control_uid, game_count, control_type, bLuckyCtrl, isStockCtrl, GetIsAllRobotOrPlayerJetton());
