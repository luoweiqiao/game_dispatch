...
bool CGameMaJiangTable::ReAnalysisParam() {
	...
	LOG_ERROR("reader json parse success - roomid:%d,tableid:%d,m_playType:%d,m_uRobotZiMoPro:%d,strDispatchCardPro:%s,param:%s",
		GetRoomID(), GetTableID(), m_playType, m_uRobotZiMoPro, strDispatchCardPro.c_str(), param.c_str());
...
bool CGameMaJiangTable::StartSendHandCard()
{
	...
	bool bSetControlCardData = SetControlCardData();
	bool bSetStockControl = SetStockWinLose(); // add by har
	if (bSetControlCardData == true)
	{
	...
	LOG_DEBUG("branker_hand_card- roomid:%d,tableid:%d,chairID:%d,uid:%d,m_wBankerUser:%d,bSetControlCardData:%d,bSetStockControl:%d,m_stockTingCardZiMoPro:%d,iHandCardCount:%d,m_cbSendCardData:0x%02X - %d,strHandCardIndex:%s",
		GetRoomID(), GetTableID(), m_wBankerUser, GetPlayerID(m_wBankerUser), m_wBankerUser, bSetControlCardData,
		bSetStockControl, m_stockTingCardZiMoPro, iHandCardCount, m_cbSendCardData, m_gameLogic.SwitchToCardIndex(m_cbSendCardData), strHandCardIndex.c_str());
...
bool    CGameMaJiangTable::DispatchCardData(uint16 curUser,bool bNotGang)
{
 ...
 //   }
    ...
    if(m_bSendStatus==true)
    {
		...
		if (bStockCtrl)
			m_cbSendCardData = cbOutSendCardData; // add by har

		bool robotTingCardZiMo = false;
		if (!LuckyTingCardZiMo && !bStockCtrl)
		{
			robotTingCardZiMo = SetRobotTingCardZiMo(curUser, cbOutSendCardData);
			if (robotTingCardZiMo && cbOutSendCardData != 0xff)
			{
				m_cbSendCardData = cbOutSendCardData;
			}
			else
		...
		LOG_DEBUG("roomid:%d,tableid:%d,uid:%d,robotTingCardZiMo:%d,LuckyTingCardZiMo:%d,bStockCtrl:%d,m_stockTingCardZiMoPro:%d",
			GetRoomID(), GetTableID(), GetPlayerID(curUser), robotTingCardZiMo, LuckyTingCardZiMo, bStockCtrl, m_stockTingCardZiMoPro);
        ...
        if(acts[i] == 5 || acts[i] == 10)//杠牌(杠听)特殊处理
        {
			...
			{//暗杠
                ...
                for(uint32 j=0;j<GangCardResult.cbCardCount;++j)
				{
					...
					//LOG_DEBUG("gang_card - uid:%d,card:0x%02X", uid,a.card());
