...
bool    CFishCfgMgr::GetFishCfgInfo(CGameRoom *pHostRoom, CGamePlayer* pPlayer, uint32 fish_id, uint32 &ret, bool &is_luck_ctrl, bool &is_luck_win)
{    
	if (pPlayer == NULL)
	{
	...
	*/
    ...
    {	
		...
		if (!is_luck_ctrl && !isStockCtrl)
		{
			...
			LOG_DEBUG(" XXXX tmp_value:%d old_kill_rate:%d new_kill_rate:%d .", tmp_value, old_kill_rate, kill_rate);
		}
		...
		LOG_DEBUG("fish is kill roomid:%d,fish_id:%d,rand_value:%d,kill_rate:%d,ret:%d,is_luck_ctrl:%d,isStockCtrl:%d,stockChangeRate:%f,jackpot:%lld,killPointsLine:%lld,jackpotMin:%lld,playerWinRate:%d,jackpotCoefficient:%d,jackpotRate:%d,realplayerWinRate:%f,jackpotSub:%f",
			room_id, fish_id, rand_value, kill_rate, ret, is_luck_ctrl, isStockCtrl, stockChangeRate,
			roomStockCfg.jackpot, roomStockCfg.killPointsLine, roomStockCfg.jackpotMin, roomStockCfg.playerWinRate, roomStockCfg.jackpotCoefficient, roomStockCfg.jackpotRate, playerWinRate, jackpotSub);
		return true;
    } 
