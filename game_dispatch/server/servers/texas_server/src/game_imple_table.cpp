...
bool CGameTexasTable::ReAnalysisParam() {
	...
	if (jvalue.isMember("rmc") && jvalue["rmc"].isIntegral()) {
	...
	LOG_DEBUG("reader_success - roomid:%d,tableid:%d,m_robotBankerMaxCardPro:%d,m_confArrDispatchCardAllPro:%d,m_iArrDispatchCardPro:%d-%d-%d-%d-%d-%d-%d-%d-%d-%d",
...
void    CGameTexasTable::SetCardDataControl()
{
	...
	if (!bIsFalgControlMulti)
	{
        ...
        //活跃福利控制       
        /*if (!bIsControlPlayerIsReady && !bIsNoviceWelfareCtrl)  将活跃福利优先级放到机器人最大牌之后 delete by har
        {
            bIsAWCtrl = ActiveWelfareCtrl();
        }*/

		if (!bIsControlPlayerIsReady && !bIsAWCtrl && !bIsNoviceWelfareCtrl && !bIsLuckyCtrl)
		{
			//玩家最多能赢
			bIsWinMaxScorePlayerLost = SetWinMaxScorePlayerLost();
		}
		
		if (!bIsControlPlayerIsReady && !bIsAWCtrl && !bIsWinMaxScorePlayerLost && !bIsNoviceWelfareCtrl && !bIsLuckyCtrl)
		{
		...
		if (!bIsControlPlayerIsReady && !bIsAWCtrl && !bIsWinMaxScorePlayerLost && !bIsNoviceWelfareCtrl && !bIsLuckyCtrl && !bSetRobotMaxCardType)
			bStockCtrl = SetStockWinLose();
	}
	SetIsAllRobotOrPlayerJetton(IsAllRobotOrPlayerJetton()); // add by har
	LOG_DEBUG("robot win - roomid:%d,tableid:%d,m_robotBankerMaxCardPro:%d,bIsControlPlayerIsReady:%d,control_uid:%d,control_type:%d,game_count:%d,bIsFalgControl:%d,bIsWinMaxScorePlayerLost:%d,bIsFalgControlMulti:%d,bIsAWCtrl:%d,bIsNoviceWelfareCtrl:%d,bSetRobotMaxCardType:%d bIsLuckyCtrl:%d,bStockCtrl:%d,GetIsAllRobotOrPlayerJetton:%d",
		GetRoomID(), GetTableID(), m_robotBankerMaxCardPro, bIsControlPlayerIsReady, control_uid, control_type, game_count, bIsFalgControl, bIsWinMaxScorePlayerLost, bIsFalgControlMulti, bIsAWCtrl, bIsNoviceWelfareCtrl, bSetRobotMaxCardType, bIsLuckyCtrl, bStockCtrl, GetIsAllRobotOrPlayerJetton());
}
