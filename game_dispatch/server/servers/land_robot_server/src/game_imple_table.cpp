...
bool CGameLandTable::ReAnalysisParam()
{
	...
	LOG_DEBUG("roomid:%d,tableid:%d,m_isaddblockers:%d,m_robotBankerWinPro:%d", GetRoomID(), GetTableID(), m_isaddblockers, m_robotBankerWinPro);
...
bool CGameLandTable::OnGameStart()
{
	...
	LOG_DEBUG("roomid:%d,tableid:%d,uid:%d %d %d,m_curUser:%d,m_firstUser:%d,startUser:%d,curUser:%d,bControlCardData:%d,m_evaluateUser:%d,GetIsAllRobotOrPlayerJetton:%d,validCardIndex:%d,validCardData:0x%02X",
		GetRoomID(), GetTableID(), GetPlayerID(0), GetPlayerID(1), GetPlayerID(2), m_curUser, m_firstUser, startUser, curUser, bControlCardData, m_evaluateUser, GetIsAllRobotOrPlayerJetton(), validCardIndex, validCardData);
    ...
    case GER_NORMAL:		//常规结束
        {
            ...
            int64 curLandScore = GetPlayerCurScore(GetPlayer(m_bankerUser))/2;// 能结算的钱
            {
                // 计算输的分数
                for(uint16 i = 0;i < GAME_LAND_PLAYER;i++)
                {
            ...
            for(uint16 i=0;i<GAME_LAND_PLAYER;++i) {
                ...
                //CalcPlayerInfo(GetPlayer(i),i, isWin, isSpring, isLand,calcScore[i]); delete by har
...
bool CGameLandTable::OnUserOutCard(uint16 chairID,uint8 cardData[],uint8 cardCount)
{
	...
	LOG_DEBUG("1 roomid:%d,tableid:%d,uid:%d,chairID:%d,robot:%d,count:%d,card:%s", 
		GetRoomID(),GetTableID(), GetPlayerID(chairID), chairID,ChairIsRobot(chairID), cardCount, strOutCards.c_str());
	LOG_DEBUG("2 roomid:%d,tableid:%d,uid:%d,chairID:%d,robot:%d,count:%d,card:%s",
		GetRoomID(), GetTableID(), GetPlayerID(chairID), chairID, ChairIsRobot(chairID), m_handCardCount[chairID], strHandCards.c_str());
    ...
    if(m_gameLogic.RemoveCardList(cardData,cardCount,m_handCardData[chairID],m_handCardCount[chairID])==false)
    {
		...
		LOG_DEBUG("removecard_error - roomid:%d,tableid:%d,uid:%d,chairID:%d,robot:%d,count:%d,card:%s", 
			GetRoomID(), GetTableID(), GetPlayerID(chairID), chairID, ChairIsRobot(chairID), cardCount, strOutCards.c_str());
		LOG_DEBUG("removecard_error - roomid:%d,tableid:%d,uid:%d,chairID:%d,robot:%d,count:%d,card:%s",
			GetRoomID(), GetTableID(), GetPlayerID(chairID), chairID, ChairIsRobot(chairID), m_handCardCount[chairID], strHandCards.c_str());
...
bool    CGameLandTable::OnSubGameStart()
{
	...
	for(WORD wChairID=0; wChairID<GAME_LAND_PLAYER; ++wChairID)
	{
		...
		//bool bInSetLevel = m_LordRobot[wChairID].RbtInSetLevel(BEST_LEVEL_2);
		//bool bInSetLevel = m_LordRobot[wChairID].RbtInSetLevel(6);
		bool bInSetLevel = false;
		if (ChairIsRobot(wChairID) == true)
		{
			bInSetLevel = m_LordRobot[wChairID].RbtInSetLevel(BEST_LEVEL_2);
		}
		else
		{
		...
		LOG_DEBUG("land_robot_ai_log 游戏开始 - roomid:%d,tableid:%d,wChairID:%d,robot:%d,uid:%d,bResetData:%d,bInSetLevel:%d,bInInitCard:%d,bInNtfCardInfo:%d,argSeat:%d,strCard:%s", GetRoomID(), GetTableID(), wChairID, ChairIsRobot(wChairID), GetPlayerID(wChairID), bResetData, bInSetLevel, bInInitCard, bInNtfCardInfo, argSeat,strCard.c_str());
...
bool	CGameLandTable::ProgressControlPalyer()
{
	...
	if (bIsFalgControlMulti)
	{
		LOG_DEBUG("roomid:%d,tableid:%d,vecControlMultiPlayer_size:%d %d,uLostPlayerUid:%d,uWinPlayerUid:%d,bIsFalgControlMulti:%d",
			GetRoomID(), GetTableID(), vecControlMultiPlayerLost.size(), vecControlMultiPlayerWin.size(), uLostPlayerUid, uWinPlayerUid, bIsFalgControlMulti);
	...
	LOG_DEBUG("roomid:%d,tableid:%d,control_uid:%d,game_count:%d,control_type:%d,bIsHaveControlPlayer:%d",
		GetRoomID(), GetTableID(), control_uid, game_count, control_type, bIsHaveControlPlayer);
...
bool	CGameLandTable::SetControlPalyerLost(uint32 control_uid)
{
	...
	if (argSeat < GAME_LAND_PLAYER)
	{
		...
		WORD wChairID = 0;
		int add_pre = 1;
		for (; wChairID < GAME_LAND_PLAYER; ++wChairID)
		{
			CGamePlayer *pPlayer = GetPlayer(wChairID);
			if (pPlayer == NULL)
			{
		...
		LOG_DEBUG("roomid:%d,tableid:%d,control_uid:%d,m_evaluateUser:%d,argSeat:%d,wChairID:%d,add_pre:%d",GetRoomID(),GetTableID(), control_uid, m_evaluateUser,argSeat, wChairID, add_pre);
		return true;
	}
