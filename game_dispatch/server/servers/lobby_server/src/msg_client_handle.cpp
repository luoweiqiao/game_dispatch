...
{
	//LOG_DEBUG("lobby server recv client msg  - uid:%d,uin:%d,cmd:%d,begin:%d,end:%d", pNetObj->GetUID(),head->uin, head->cmd, net::CMD_MSG_EVERY_COLOR_GAME_BEGIN, net::CMD_MSG_EVERY_COLOR_GAME_END);
...
#define HANDLE_CLIENT_FUNC(cmd,handle) \
	...
	HANDLE_CLIENT_FUNC(net::S2C_MSG_NOTIFY_VIP_RECHARGE_REQ, handle_msg_notify_vip_recharge_req);

	HANDLE_CLIENT_FUNC(net::S2C_MSG_NOTIFY_UNION_PAY_RECHARGE_REQ, handle_msg_notify_union_pay_recharge_req);
	HANDLE_CLIENT_FUNC(net::S2C_MSG_NOTIFY_WECHAT_PAY_RECHARGE_REQ, handle_msg_notify_wechat_pay_recharge_req);
	HANDLE_CLIENT_FUNC(net::S2C_MSG_NOTIFY_ALI_PAY_RECHARGE_REQ, handle_msg_notify_ali_pay_recharge_req);
	HANDLE_CLIENT_FUNC(net::S2C_MSG_NOTIFY_OTHER_PAY_RECHARGE_REQ, handle_msg_notify_other_pay_recharge_req);
	HANDLE_CLIENT_FUNC(net::S2C_MSG_NOTIFY_QQ_PAY_RECHARGE_REQ, handle_msg_notify_qq_pay_recharge_req);
	HANDLE_CLIENT_FUNC(net::S2C_MSG_NOTIFY_WECHAT_SCAN_PAY_RECHARGE_REQ, handle_msg_notify_wechat_scan_pay_recharge_req);
	HANDLE_CLIENT_FUNC(net::S2C_MSG_NOTIFY_JD_PAY_RECHARGE_REQ, handle_msg_notify_jd_pay_recharge_req);
	HANDLE_CLIENT_FUNC(net::S2C_MSG_NOTIFY_APPLE_PAY_RECHARGE_REQ, handle_msg_notify_apple_pay_recharge_req);
...
int  CHandleClientMsg::route_to_everycolor_game_svr(NetworkObject* pNetObj, const uint8* pkt_buf, uint16 buf_len, PACKETHEAD* head)
{
	...
	if (svrID != 0 && pPlayer != NULL) {
		...
		LOG_DEBUG("everycolor server do not find - svrID:%d,uid:%d", svrID, pPlayer->GetUID());
	}
	return 0;
}
...
int	 CHandleClientMsg::handle_msg_login(NetworkObject* pNetObj, const uint8* pkt_buf, uint16 buf_len)
{
	...
	uint16 PlayerServerID = CRedisMgr::Instance().GetPlayerLobbySvrID(msg.uid());
	uint16 curServerID = CApplication::Instance().GetServerID();

	LOG_DEBUG("player login game server recv msg - uid:%d,PlayerServerID:%d,curServerID:%d,,tempUID:%d,deviceid:%s,key:%s,IP:%s,strCity:%s",msg.uid(), PlayerServerID, curServerID,tempUID,msg.deviceid().c_str(),msg.key().c_str(), chTmpIP, strCity.c_str());
	uint32 uid 		 = msg.uid();
	if (uid == 0)
	{
		return 0;
	...
	else
	{
		if(pPlayerUid != NULL)// 在玩
		{
			...
			LOG_ERROR("push against - uid:%d,oldSock:%p,newSock:%p ", uid, pOldSock, pNetObj);
...
int  CHandleClientMsg::handle_msg_give_safebox(NetworkObject* pNetObj, const uint8* pkt_buf, uint16 buf_len)
{
	...
	gcoin -= tax;  
	uint32 pTarPlayerLobbysvrID = CRedisMgr::Instance().GetPlayerLobbySvrID(guid);
	if(pTarPlayer != NULL)
	{
	...
	else if(pTarPlayerLobbysvrID != 0 && pTarPlayerLobbysvrID != CApplication::Instance().GetServerID())
    {
        //该玩家在另一个大厅
        net::msg_give_safebox_req dispatchMsg;
        dispatchMsg.set_give_uid(guid);
        dispatchMsg.set_give_coin(gcoin);
        dispatchMsg.set_own_uid(pPlayer->GetUID());
        DispatchMgr::Instance().Broadcast2OtherLobby(&dispatchMsg, net::C2S_MSG_GIVE_SAFEBOX, pTarPlayerLobbysvrID, CApplication::Instance().GetServerID());
    }
	else
	{
		...
		case net::REWARD_CLOGIN:
			{
                if(pPlayer->GetDayGameCount() < CDataCfgMgr::Instance().GetSignGameCount())
                {
                    ...
                    string title    = "奖励提示"; 
                    string content  = CStringUtility::FormatToString("您已获得 %d 天签到奖励，奖金是 %.2F 金币 (备注：7天及以后签到是 %.2F 金币)",pPlayer->GetCLogin(),(coin*0.01),(CDataCfgMgr::Instance().GetCLoginScore(7)*0.01)); 
...
int  CHandleClientMsg::handle_msg_notify_vip_recharge_req(NetworkObject* pNetObj, const uint8* pkt_buf, uint16 buf_len)
{
	...
	LOG_DEBUG("vip recharge show req 2 - uid:%d", pNetObj->GetUID());
...
int  CHandleClientMsg::handle_msg_notify_union_pay_recharge_req(NetworkObject* pNetObj, const uint8* pkt_buf, uint16 buf_len)
{
	...
	LOG_DEBUG("pay recharge show req 2 - uid:%d", pNetObj->GetUID());
...
int CHandleClientMsg::handle_msg_chat_info_forward_req(NetworkObject *pNetObj, const uint8 *pkt_buf, uint16 buf_len) {
	...
	for (int i = 0; i < toid_size; ++i) {
		...
		if (pPlayer2 == NULL) {
			uint16 PlayerServerID = CRedisMgr::Instance().GetPlayerLobbySvrID(uid);
			if (PlayerServerID == 0) {
				msg.add_online(0);
				msg.add_toname(name);
			} else {
				SPlayerData *pdata = CPlayerMgr::Instance().GetOtherLobbyPlayerData(uid);
				if (pdata == NULL) {
					msg.add_online(0);
					msg.add_toname(name);
					LOG_ERROR("pdata == NULL fromid:%d,mtype:%d,messagStr:%s,uiTime:%lld,uid:%d",
						pPlayer->GetUID(), msg.mtype(), msg.messagstr().c_str(), uiTime, uid);
					...
					msg.add_toname(name);
					msg.set_duid(uid);
					// 该玩家在另一个大厅,通知该玩家所在的大厅服(通过中心服转发)，发送玩家消息
					DispatchMgr::Instance().Broadcast2OtherLobby(&msg, net::S2C_MSG_CHAT_INFO_FORWARD_REP, PlayerServerID, CApplication::Instance().GetServerID());
					LOG_DEBUG("other lobby fromid:%d,mtype:%d,messagStr:%s,uiTime:%lld,uid:%d",
						pPlayer->GetUID(), msg.mtype(), msg.messagstr().c_str(), uiTime, uid);
				}
			}
		}
	...
	LOG_DEBUG("fromid:%d,mtype:%d,messagStr:%s,uiTime:%lld,str:%s", pPlayer->GetUID(), msg.mtype(), msg.messagstr().c_str(), uiTime, str.c_str());
	return 0;
}
